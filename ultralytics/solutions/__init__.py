...
def patch_streamlit_watcher():
    ...
    try:
        ...
        lsw.get_module_paths = lambda m: [] if getattr(m, "__name__", "").startswith("torch.classes") else (
            lsw._orig_get_module_paths(m) if callable(lsw._orig_get_module_paths) else []
        )
    except Exception:
        pass
