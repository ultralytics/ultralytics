...
Managed training works through the normal training flow. Workers download the pinned originals into temporary job storage for the run and remove them with job cleanup — your AWS credentials never reach compute, because Platform hands the worker short-lived signed links to its own image endpoint instead.
