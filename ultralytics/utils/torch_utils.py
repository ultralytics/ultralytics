...
def get_flops(model, imgsz=640, should_forward_one2many_head=False):
    ...
    try:
        ...
        try:
            ...
            inputs = [im, False, False, False, None, should_forward_one2many_head] # to avoid 'v10Pose' object has no attribute 'cv4' error (parameters of nn > tasks.py > BaseModel > predict())
            flops = thop.profile(deepcopy(model), inputs=inputs, verbose=False)[0] / 1e9 * 2  # stride GFLOPs
            return flops * imgsz[0] / stride * imgsz[1] / stride  # imgsz GFLOPs
        except Exception:
            # Use actual image size for input tensor (i.e. required for RTDETR models)
            im = torch.empty((1, p.shape[1], *imgsz), device=p.device)  # input image in BCHW format
            inputs = [im, False, False, False, None, should_forward_one2many_head] # to avoid 'v10Pose' object has no attribute 'cv4' error (parameters of nn > tasks.py > BaseModel > predict())
            return thop.profile(deepcopy(model), inputs=inputs, verbose=False)[0] / 1e9 * 2  # imgsz GFLOPs
    except Exception:
        return 0.0
