...
def download_model_via_yolo(model_name: str, destination: str) -> bool:
    """
    Downloads a YOLO model using the built-in downloader via torch.hub from ultralytics/yolov5. Loads the model (which
    triggers the download if necessary) and then copies the cached file to the 'yolo' folder.
...
def download_model_generic(model_name: str, destination: str) -> bool:
    """
    Determines the download method based on the model name. For models starting with 'yolov5', uses the built-in
    downloader. For other versions (e.g., 'yolo11n'), prompts the user for a download URL.
...
def export_model_to_ncnn(model_path: str) -> bool:
    """
    Exports the YOLO model to NCNN format using the built-in export() method provided by the Ultralytics YOLO class. The
    exported files are saved in a folder adjacent to the model file (e.g., 'yolo11n_ncnn_model').
