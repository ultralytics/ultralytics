...
function renderOpenvinoChart(selectedDevice) {
  ...
  const precisions = isCPU ? ['PyTorch', 'FP32', 'INT8', 'FP16'] : ['FP32', 'INT8', 'FP16'];
  const groupedData = {};
  ...
  models.forEach(model => {
    ...
    if (isCPU) {  // Add PyTorch only for CPU
      const pytorch = entries.find(e => e.format === 'pytorch' && e.precision === 'FP32' && e.device === 'cpu');
      groupedData.PyTorch.push(pytorch ? pytorch.inference_speed : null);
    }

    ['FP32', 'INT8', 'FP16'].forEach(p => {
      const item = entries.find(e => e.format === 'openvino' && e.device === selectedDevice && e.precision === p);
      groupedData[p].push(item ? item.inference_speed : null);
    });
  });

  const datasets = precisions.map(p => ({label: p, data: groupedData[p], backgroundColor: colormap[p]}));
  const labels = models.map(m => m);

  openvinoComparisonChart = new Chart(document.getElementById('openvinoChart').getContext('2d'), {
    type: 'bar',
    data: {
      labels,
      datasets
    ...
    options: {
          ...
          callbacks: {
            label: ctx => {
              ...
              if (label === 'PyTorch') {
                ...
                match = data[model].find(e => e.format === 'openvino' && e.device === selectedDevice && e.precision === label);
              }
      ...
      scales: {
        ...
        y: {
          beginAtZero: true,
          title: { display: true, text: 'Inference Time (ms)', color: '#fff' },
          ticks: { color: '#fff' },
          grid: { color: 'rgba(255,255,255,0.1)' }
        }
      }
