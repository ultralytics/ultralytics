...
PyTorch and dynamic ONNX/Triton models use [minimum-rectangle padding](../modes/predict.md#fixed-shape-vs-minimum-rectangle-rect), while static exports pad to the full `imgsz`. Use `rect=False` for native inference to match a static export, or export with `dynamic=True` where supported.
