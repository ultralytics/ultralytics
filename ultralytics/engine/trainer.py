...
class BaseTrainer:
    ...
    def build_optimizer(self, model, name="auto", lr=0.001, momentum=0.9, decay=1e-5, iterations=1e5):
        ...
        len_params = [len(g[0]), len(g[1]), len(g[2])]  # lengths of param groups
        ...
        if use_muon:
            len_params[0] = len(g[3])  # update number of params
        ...
        LOGGER.info(
            ...
            f"{len_params[1]} weight(decay=0.0), {len_params[0]} weight(decay={decay}), {len_params[2]} bias(decay=0.0)"
        )
        return optimizer
