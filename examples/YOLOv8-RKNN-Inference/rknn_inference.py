...
Supports the *separate-head* model layout produced by `export_onnx.py`:
  - 6-output: [box, Sigmoid(cls)] x 3 scales
  - 9-output: [box, cls_logit, score_sum] x 3 scales
The layout is detected automatically from the model's output count.
...
def post_process(outputs, conf=OBJ_THRESH, nms=NMS_THRESH):
    ...
    outputs: fp32 tensors, either [box, cls, score_sum] x 3 (9-output, cls is
             logit) or [box, cls] x 3 (6-output, cls is already Sigmoid).
    Returns boxes in the 640x640 letterbox space, or (None, None, None).
    """
...
def unmap_box(boxes, ratio, pad):
    """Map 640x640 letterbox coordinates back to the original image."""
    dw, dh = pad
