...
class Results(SimpleClass, DataExportMixin):
    ...
    def __init__(
        ...
        Args:
            ...
            depths (torch.Tensor | np.ndarray | None): Depth values for MDE task, shape (N,) for N detections.
        ...
        self.depths = depths  # Depth values for MDE task
    ...
    def plot(
        ...
        if pred_boxes is not None and show_boxes:
            # Prepare depth values if available (for MDE task)
            depth_vals = None
            if self.depths is not None:
                depth_vals = self.depths.cpu().numpy() if isinstance(self.depths, torch.Tensor) else self.depths
                if depth_vals.ndim == 2:
                    depth_vals = depth_vals.squeeze(-1)  # Remove last dimension if (N, 1)
            ...
            for i, d in enumerate(reversed(pred_boxes)):
                ...
                if depth_vals is not None and labels:
                    depth_idx = len(pred_boxes) - 1 - i  # Reverse index for depth values
                    if depth_idx < len(depth_vals):
                        depth_val = float(depth_vals[depth_idx])
                        label = f"{name} {d_conf:.2f} {depth_val:.2f}m" if conf else f"{name} {depth_val:.2f}m"
                    else:
