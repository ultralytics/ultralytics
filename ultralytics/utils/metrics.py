...
def probiou(obb1, obb2, eps=1e-7):
    ...
    def _get_gbb(boxes):
        """Generate Gaussian bounding boxes from obbs."""
        return torch.cat((boxes[:, :2], torch.pow(boxes[:, 2:4], 2) / 12, boxes[:, 4:]), dim=-1)

    def _get_rotated(a, b, r):
        return (
            a * torch.cos(r) ** 2 + b * torch.sin(r) ** 2,
            a * torch.sin(r) ** 2 + b * torch.cos(r) ** 2,
            a * torch.cos(r) * torch.sin(r) - b * torch.sin(r) * torch.cos(r),
        )

    gbboxes1 = _get_gbb(obb1)
    gbboxes2 = _get_gbb(obb2)
    x1, y1, a1_, b1_, c1_ = gbboxes1.split(1, dim=-1)
    x2, y2, a2_, b2_, c2_ = gbboxes2.split(1, dim=-1)
    a1, b1, c1 = _get_rotated(a1_, b1_, c1_)
    a2, b2, c2 = _get_rotated(a2_, b2_, c2_)
    ...
    t3 = (
        torch.log(
            ...
            / (4 * torch.sqrt((a1 * b1 - torch.pow(c1, 2)) * (a2 * b2 - torch.pow(c2, 2))) + eps)
            + eps
        )
        * 0.5
    )
