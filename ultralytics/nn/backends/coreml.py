...
class CoreMLBackend(BaseBackend):
    ...
    def load_model(self, weight: str | Path) -> None:
        ...
        # Run on the Neural Engine (CPU_AND_NE): ~3x faster than CPU. Needs macOS >= 13, so fall back to
        # CPU_ONLY below that. CoreML inference is macOS-only, so this applies wherever the backend runs.
