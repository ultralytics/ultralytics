...
| `args`    | object | No          | Export options: `imgsz`, `quantize`, `dynamic`, `simplify`, `opset`, `conf`, `iou`, `batch`, `workspace`, `nms`, `end2end` (deprecated), `optimize`, `keras`, and `name` (device target for RKNN, QNN, Hailo, and Ascend formats) |
