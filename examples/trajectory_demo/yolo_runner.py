...
def run(source: str, weights: str, output: str, auto_subdir: bool = True, conf_threshold: float = 0.5, use_segmentation: bool = False, homography_path: str = None):
...
def generate_analysis_report(tracks_path: str, near_misses_path: str, output_dir: str, fps: float = 30.0, distance_unit: str = "pixels") -> None:
    """生成综合分析报告。
    ...
    if collision_risks:
        ...
        for i, nm in enumerate(sorted_risks[:10], 1):
            ...
            dist_str = f"{nm['distance']:.2f}{distance_unit[0].upper()}" if distance_unit == "pixels" else f"{nm['distance']:.2f}m"
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        if ttcs:
            f.write(f"Time-to-Collision (TTC) Statistics (seconds):\n")
...
if __name__ == '__main__':
    ...
    p.add_argument('--no-auto-subdir', action='store_true', 
                   help='不根据视频名称和时间戳创建子文件夹（覆盖模式）')
    p.add_argument('--segmentation', action='store_true', help='使用YOLO分割模型（更精确的边界）')
    p.add_argument('--homography', type=str, default=None, 
                   help='Homography矩阵JSON文件路径（用于像素->世界坐标变换）')
    args = p.parse_args()
    
    # 运行主程序
    run(args.source, args.weights, args.output, auto_subdir=not args.no_auto_subdir, 
        conf_threshold=args.conf, use_segmentation=args.segmentation, homography_path=args.homography)
