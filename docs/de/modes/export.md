...
## comments: true description: Schritt-für-Schritt-Anleitung zum Exportieren Ihrer YOLOv8-Modelle in verschiedene Format wie ONNX, TensorRT, CoreML und mehr für den Einsatz. keywords: YOLO, YOLOv8, Ultralytics, Model-Export, ONNX, TensorRT, CoreML, TensorFlow SavedModel, OpenVINO, PyTorch, Model exportieren
...
Das ultimative Ziel des Trainierens eines Models besteht darin, es für reale Anwendungen einzusetzen. Der Exportmodus in Ultralytics YOLOv8 bietet eine vielseitige Palette von Optionen für den Export Ihres trainierten Models in verschiedene Format, sodass es auf verschiedenen Plattformen und Geräten eingesetzt werden kann. Dieser umfassende Leitfaden soll Sie durch die Nuancen des Modell-Exports führen und zeigen, wie Sie maximale Kompatibilität und Leistung erzielen können.
...
- **Vielseitigkeit:** Export in verschiedene Format einschließlich ONNX, TensorRT, CoreML und mehr.
- **Leistung:** Bis zu 5-fache GPU-Beschleunigung mit TensorRT und 3-fache CPU-Beschleunigung mit ONNX oder OpenVINO.
- **Kompatibilität:** Machen Sie Ihr Model universell einsetzbar in zahlreichen Hardware- und Softwareumgebungen.
...
- **Ein-Klick-Export:** Einfache Befehle für den Export in verschiedene Format.
...
=== "Python"
    ...
    # Laden eines Models
    model = YOLO('yolov8n.pt')  # offizielles Model laden
    model = YOLO('path/to/best.pt')  # benutzerdefiniertes trainiertes Model laden
...
=== "CLI"
    ...
    yolo export model=path/to/best.pt format=onnx  # benutzerdefiniertes trainiertes Model exportieren
    ```
````

## Argumente

Exporteinstellungen für YOLO-Modelle beziehen sich auf verschiedene Konfigurationen und Optionen, die verwendet werden, um das Model zu speichern oder für den Einsatz in anderen Umgebungen oder Plattformen zu exportieren. Diese Einstellungen können die Leistung, Größe und Kompatibilität des Models mit verschiedenen Systemen beeinflussen. Zu den gängigen Exporteinstellungen von YOLO gehören das Format der exportierten Modelldatei (z. B. ONNX, TensorFlow SavedModel), das Gerät, auf dem das Model ausgeführt wird (z. B. CPU, GPU) und das Vorhandensein zusätzlicher Funktionen wie Masken oder mehrere Labels pro Box. Andere Faktoren, die den Exportprozess beeinflussen können, sind die spezifische Aufgabe, für die das Model verwendet wird, und die Anforderungen oder Einschränkungen der Zielumgebung oder -plattform. Es ist wichtig, diese Einstellungen sorgfältig zu berücksichtigen und zu konfigurieren, um sicherzustellen, dass das exportierte Model für den beabsichtigten Einsatzzweck optimiert ist und in der Zielumgebung effektiv eingesetzt werden kann.
