...
!!! abstract "Summary"
    ...
    === "<span class="doc-kind doc-kind-class">Classes</span>"
        ...
        - [`ReID`](#ultralytics.trackers.utils.reid.ReID)

    === "<span class="doc-kind doc-kind-method">Methods</span>"

        - [`ReID._crop_detections`](#ultralytics.trackers.utils.reid.ReID._crop_detections)
        - [`ReID._crops_to_tensor`](#ultralytics.trackers.utils.reid.ReID._crops_to_tensor)
        - [`ReID.__call__`](#ultralytics.trackers.utils.reid.ReID.__call__)

    === "<span class="doc-kind doc-kind-function">Functions</span>"

        - [`build_encoder`](#ultralytics.trackers.utils.reid.build_encoder)
        - [`smooth_feature`](#ultralytics.trackers.utils.reid.smooth_feature)


## Class `ultralytics.trackers.utils.reid.ReID` {#ultralytics.trackers.utils.reid.ReID}

```python
ReID(model: str, imgsz: int = 224, device: str | torch.device | None = None, fp16: bool = False)
```
...
| --- | --- | --- | --- |
| `model` | `str` | Path to a ReID model. `.pt` runs through the YOLO predictor (embed-layer extraction); other extensions go through `AutoBackend`. | *required* |
...
| `device` | `str \| torch.device \| None` | Inference device; defaults to CUDA if available. | `None` |
...
| Name | Description |
| --- | --- |
| [`__call__`](#ultralytics.trackers.utils.reid.ReID.__call__) | Extract embeddings for detected objects. |
| [`_crop_detections`](#ultralytics.trackers.utils.reid.ReID._crop_detections) | Crop detection regions from image, converting xywh to xyxy first. |
| [`_crops_to_tensor`](#ultralytics.trackers.utils.reid.ReID._crops_to_tensor) | Stack a list of valid image crops into a normalized BCHW float tensor at self.imgsz. |

<details>
<summary>Source code in <code>ultralytics/trackers/utils/reid.py</code></summary>

<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/trackers/utils/reid.py#L23-L124">View on GitHub</a>
```python
class ReID:
    """ReID encoder. Routes `.pt` to the YOLO predictor path; everything else to `AutoBackend`."""
...
### Method `ultralytics.trackers.utils.reid.ReID.__call__` {#ultralytics.trackers.utils.reid.ReID.\_\_call\_\_}
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/trackers/utils/reid.py#L95-L124">View on GitHub</a>
```python
@torch.no_grad()
def __call__(self, img: np.ndarray, dets: np.ndarray) -> list[np.ndarray | None]:
    """Extract embeddings for detected objects."""
...
### Method `ultralytics.trackers.utils.reid.ReID._crop_detections` {#ultralytics.trackers.utils.reid.ReID.\_crop\_detections}
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/trackers/utils/reid.py#L71-L81">View on GitHub</a>
```python
@staticmethod
def _crop_detections(img: np.ndarray, dets: np.ndarray) -> list[np.ndarray]:
    """Crop detection regions from image, converting xywh to xyxy first.
...
### Method `ultralytics.trackers.utils.reid.ReID._crops_to_tensor` {#ultralytics.trackers.utils.reid.ReID.\_crops\_to\_tensor}
...
| Name | Type | Description | Default |
| --- | --- | --- | --- |
| `crops` | `list[np.ndarray]` |  | *required* |

<details>
<summary>Source code in <code>ultralytics/trackers/utils/reid.py</code></summary>

<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/trackers/utils/reid.py#L83-L92">View on GitHub</a>
```python
def _crops_to_tensor(self, crops: list[np.ndarray]) -> torch.Tensor:
    """Stack a list of valid image crops into a normalized BCHW float tensor at self.imgsz."""
...
## Function `ultralytics.trackers.utils.reid.build_encoder` {#ultralytics.trackers.utils.reid.build\_encoder}
...
| `model` | `str \| None` | `"auto"` returns a callable that converts pre-extracted backbone features to numpy arrays; any other value loads a `ReID` model from that path. Ignored when `with_reid` is False. | *required* |
| `device` | `str \| torch.device \| None` | Inference device for the ReID model; defaults to CUDA if available. | `None` |
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/trackers/utils/reid.py#L127-L149">View on GitHub</a>
```python
def build_encoder(with_reid: bool, model: str | None, device: str | torch.device | None = None):
    """Return a ReID encoder, the native-features pass-through, or None.
...
## Function `ultralytics.trackers.utils.reid.smooth_feature` {#ultralytics.trackers.utils.reid.smooth\_feature}
...
| `smooth` | `np.ndarray \| None` | Current smoothed feature, or None on the first update. | *required* |
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/trackers/utils/reid.py#L152-L174">View on GitHub</a>
```python
def smooth_feature(
    feat: np.ndarray, smooth: np.ndarray | None, alpha: float
) -> tuple[np.ndarray | None, np.ndarray | None]:
