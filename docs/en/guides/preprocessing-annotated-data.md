...
Libraries like [OpenCV](https://www.ultralytics.com/glossary/opencv) and PIL (Pillow) provide these functions, but with YOLO26 you usually don't resize manually. The `imgsz` argument handles it. On the default path every image reaches the model at a square `imgsz × imgsz` resolution such as `640 × 640`, and how it gets there depends on the mode: during [validation](../modes/val.md) and [inference](../modes/predict.md) YOLO letterboxes each image, scaling it down so its largest dimension matches `imgsz` while preserving the aspect ratio, then padding the shorter side with gray (value 114). Images already smaller than `imgsz` are padded rather than enlarged. On the default [training](../modes/train.md) path, `mosaic=1.0` instead tiles four images onto a larger canvas and crops back to `imgsz`; once `close_mosaic` disables mosaic for the final epochs, images are letterboxed and then still randomly transformed rather than passed through untouched. That square default is not universal: rectangular batches (`rect=True`) and `multi_scale` change the input shape.
...
Yes. On the default path the `imgsz` argument gives every image the same square input size with no manual work. During validation and inference, YOLO letterboxes each image, scaling it down so its largest dimension matches the specified size (e.g., 640 pixels) while preserving the aspect ratio, then padding the shorter side; images smaller than that size are padded rather than enlarged. During training, the default mosaic augmentation reaches the same size by tiling four images and cropping instead. Rectangular batches (`rect=True`) and `multi_scale` are the exceptions to the square default. See the [model training](../modes/train.md) documentation for details.
