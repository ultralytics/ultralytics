...
2. Batch reviews before the final gate: record `git rev-parse HEAD`, then inside the implementation session launch focused reviewer children sharing context to review that same SHA for correctness, performance and production readiness, and Core Principles/minimalism; every reviewer must continue after the first blocker and report all findings. Fix the batch, then have the same children review `<recorded-sha>..HEAD` plus affected invariants, advancing the recorded SHA after each clean delta. Once clean, run one fresh `codex exec` cold-context review of the exact full PR diff at the current head; it must continue after blockers, report all findings, explicitly answer the Review gate, and say LGTM only if none remain. Restart the full review only when the head changes, never once per finding.
