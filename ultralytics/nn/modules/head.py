...
class HumanDetect(Detect):
    """YOLO-Human head predicting human attributes alongside detection boxes.

    Extends the Detect head with five extra branches predicting weight, height, age (DFL-regressed) plus gender and
    ethnicity (classified) for every anchor.

    Attributes:
        cv4 (nn.ModuleList): Convolution layers predicting weight distributions.
        cv5 (nn.ModuleList): Convolution layers predicting height distributions.
        cv6 (nn.ModuleList): Convolution layers predicting age distributions.
        cv7 (nn.ModuleList): Convolution layers predicting gender (2 classes).
        cv8 (nn.ModuleList): Convolution layers predicting ethnicity (6 classes).

    Methods:
        forward: Perform forward pass and return detections plus human attributes.
        decode_attributes: Decode the raw attribute predictions into physical units and probabilities.
        bias_init: Initialize detection and attribute head biases.

    Examples:
        Create a human detection head
        >>> human = HumanDetect(nc=1, ch=(256, 512, 1024))
        >>> x = [torch.randn(1, 256, 80, 80), torch.randn(1, 512, 40, 40), torch.randn(1, 1024, 20, 20)]
        >>> outputs = human(x)
    """

    def __init__(self, nc: int = 80, reg_max: int = 16, end2end: bool = False, ch: tuple = ()):
        """Initialize the YOLO-Human head.
        ...
        self.cv4 = nn.ModuleList(  # weight
            nn.Sequential(Conv(x, c4, 3), Conv(c4, c4, 3), nn.Conv2d(c4, self.reg_max, 1)) for x in ch
        )
        self.cv5 = nn.ModuleList(  # height
            nn.Sequential(Conv(x, c4, 3), Conv(c4, c4, 3), nn.Conv2d(c4, self.reg_max, 1)) for x in ch
        )
        self.cv6 = nn.ModuleList(  # age
            nn.Sequential(Conv(x, c4, 3), Conv(c4, c4, 3), nn.Conv2d(c4, self.reg_max, 1)) for x in ch
        )
        # gender, 2 classes
    ...
    def forward(self, x: list[torch.Tensor]):
        """Return detection predictions concatenated with the decoded human attributes."""
        bs = x[0].shape[0]  # batch size
        attributes = {
        ...
        if self.training:
            preds["attributes"] = attributes
            return preds
        pred_attributes = self.decode_attributes(**attributes)
        if self.export:
            return torch.cat([preds, pred_attributes], 1)
        y, preds = preds
        preds["attributes"] = attributes
        return torch.cat([y, pred_attributes], 1), preds
    ...
    def decode_attributes(
        ...
        """Decode raw attribute predictions into physical units and class probabilities."""
        weight = self.dfl(weight) * 12.5  # 0-200kg
        height = self.dfl(height) * 16  # 0-250cm
        age = self.dfl(age) * 6.25  # 0-100 years
        return torch.cat([weight, height, age, gender.softmax(1), ethnicity.softmax(1)], dim=1)

    def bias_init(self):
        """Initialize HumanDetect() biases, WARNING: requires stride availability."""
        super().bias_init()
        for heads in (self.cv4, self.cv5, self.cv6, self.cv7, self.cv8):
            for head in heads:
                head[-1].bias.data[:] = 1.0  # human attributes
