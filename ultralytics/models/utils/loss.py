...
class CoVMSELoss(nn.Module):
    def __init__(self, dim: int = 0, reduction: str = "mean", loss_weight: float = 1.0, eps: float = 1e-6) -> None:
        super().__init__()
        self.dim = dim
    ...
    def forward(
        ...
        loss = self.loss_weight * MSELoss(cov, target, weight, reduction=reduction, avg_factor=avg_factor)
        return loss
