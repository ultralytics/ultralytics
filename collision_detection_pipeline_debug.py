...
class DebugCollisionDetectionPipeline:
    def __init__(self, video_path, homography_path, output_base='../../results', 
        ...
        for dir_path in [self.homography_dir, self.warped_video_dir, 
                         self.yolo_dir, self.collision_dir]:
    ...
    def load_homography(self):
        ...
        print(f"  世界坐标范围:")
        print(f"    X: [{min(w[0] for w in self.world_points):.2f}, "
              f"{max(w[0] for w in self.world_points):.2f}]m")
        print(f"    Y: [{min(w[1] for w in self.world_points):.2f}, "
              f"{max(w[1] for w in self.world_points):.2f}]m")
    ...
    def create_verification_image(self):
        ...
        for i, (px, py) in enumerate(pixel_points):
            ...
            cv2.putText(frame_marked, label, (int(px)+15, int(py)-10),
    ...
    def detect_and_visualize(self, conf_threshold=0.45):
        ...
        for result in model.track(source=self.warped_video_path, stream=True,
            ...
            # 添加帧信息文本
            cv2.putText(frame, f"Frame: {frame_idx} | Objects: {len(boxes)}", 
        ...
        # 生成YOLO统计报告
        self.generate_yolo_report(class_counts, frames_with_detections, 
                                  detected_frames_info, keyframes)
        
        return detected_frames_info, keyframes
    
    def generate_yolo_report(self, class_counts, frames_with_detections,
                            detected_frames_info, keyframes):
        """生成YOLO检测统计报告"""
    ...
    def analyze_collisions(self, detected_frames_info):
        ...
        for frame_info in detected_frames_info:
            ...
            for i in range(len(detections)):
                for j in range(i+1, len(detections)):
                    ...
                    if distance < 0.5:
                        ...
                        print(f"  ⚠️  COLLISION: Frame {frame_num}, "
            ...
            if (frame_info == detected_frames_info[-1] or 
                detected_frames_info.index(frame_info) % 10 == 0):
    ...
    def save_collision_results(self, collision_events, near_miss_events):
        ...
        with open(report_path, 'w') as f:
            ...
            f.write(f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"分析范围: <0.5m (碰撞), <1.5m (接近)\n\n")
    ...
    def run(self, conf_threshold=0.45):
        ...
        try:
            ...
            # 步骤3: YOLO检测
            detected_frames_info, keyframes = self.detect_and_visualize(conf_threshold)
            self.pause_checkpoint("3_yolo_detection")
            
            # 步骤4: 碰撞分析
            if detected_frames_info:
                collision_events, near_miss_events = self.analyze_collisions(detected_frames_info)
            ...
            print(f"\n文件夹结构:")
            print(f"  1_homography/          - 标定验证")
            print(f"  2_warped_video/        - 鸟瞰图视频")
            print(f"  3_yolo_detection/      - YOLO检测结果")
            print(f"    ├── yolo_detection_viz_*.mp4       (带框视频)")
            print(f"    ├── yolo_detection_report.txt      (检测报告)")
            print(f"    └── keyframe_*.jpg                 (关键帧)")
            print(f"  4_collision_analysis/  - 碰撞分析")
            print(f"    ├── collision_events.json          (碰撞事件)")
            print(f"    ├── near_miss_events.json          (接近事件)")
            print(f"    └── collision_analysis_report.txt  (分析报告)")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--frame-skip', type=int, default=1, 
                       help='跳帧数（1=不跳，10=每10帧处理1帧，加快速度）')
    ...
    pipeline = DebugCollisionDetectionPipeline(
        args.video, args.homography, args.output,
        frame_skip=args.frame_skip,
        debug_mode=(not args.no_pause)
    )
