    ...
    def generate_train_script(self):
        ...
        if self.trainType == "Init":  # 初始化训练
            if self.netType.startswith("yolov5"):
                file.write(f"model_cfg = '{pretrained_models}/{self.netType}u.yaml'\n")
                file.write(f"model_path = '{pretrained_models}/{self.netType}u.pt'\n")
            else:
                file.write(f"model_cfg = '{pretrained_models}/{self.netType}.yaml'\n")
                file.write(f"model_path = '{pretrained_models}/{self.netType}.pt'\n")
        ...
        file.write("model_rename_path = '/'.join(['export_results',model_rename])\n")
        file.write("shutil.copy(model_trained_path,model_rename_path)\n")
        file.close()

    def train(self, re_fmt):
        self.rds.hset(f"{self.taskId}_{self.taskName}_train_progress", mapping={"status": 1, "epoch": 0})
        log_file = open(f"./log/train_log_{self.taskId}_{self.taskName}.txt", "w", encoding="utf-8")
        ...
        if not psutil.pid_exists(process.pid):  # 后台程序出错
            ...
            self.func_call_back(
                ...
                self.taskName,
                1,
                f"当前训练任务:{self.taskId}_{self.taskName}启动成功,后台进程已启动.",
                create_time,
            )
            ...
            self.rds.hset(f"{self.taskId}_{self.taskName}_train_progress", mapping={"status": status, "epoch": 0})
            if not psutil.pid_exists(process.pid):  # 后台进程已结束
                ...
                self.rds.xadd(
                    ...
                    {"result": f"当前训练任务:{self.taskId}_{self.taskName}正常结束"},
                    maxlen=100,
                )
            else:
                ...
                self.rds.xadd(
                    train_result_topic_name,
                    {"result": f"当前训练任务:{self.taskId}_{self.taskName}异常退出"},
                    maxlen=100,
                )
