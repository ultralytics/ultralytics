...
!!! warning "EdgeTPU Export Deprecated"

    **EdgeTPU export (`format=edgetpu`) is deprecated in Ultralytics.** The EdgeTPU compiler is incompatible with TFLite models generated by ai-edge-torch (our new TFLite export backend).

    **For Raspberry Pi deployment, use TFLite format instead:**

    ```bash
    yolo export model=yolo11n.pt format=tflite int8=True data=coco8.yaml
    ```

    The exported TFLite model can run efficiently on Raspberry Pi without EdgeTPU hardware. If you have existing EdgeTPU models or need to use EdgeTPU hardware, consider using older Ultralytics versions or manually creating compatible TFLite models.
