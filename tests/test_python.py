...
@pytest.mark.parametrize("status", [200, 401, 403, 404, 408, 429, 500])
def test_platform_job_transport(tmp_path, status):
    """Exercise SDK callback retries, credentials and cancellation responses over loopback HTTP."""
    import json
    import subprocess
    import sys
    ...
    pytest.importorskip("ultralytics_platform")
    captured = []

    class Receiver(BaseHTTPRequestHandler):
        def do_POST(self):
            payload = json.loads(self.rfile.read(int(self.headers["Content-Length"])))
            captured.append((self.path, self.headers["Authorization"], payload))
            response = {"received": True, "cancelled": True}
...
platform._api_key = os.environ["ULTRALYTICS_API_KEY"]
result = platform._send("epoch_end", {{"epoch": 0, "loss": float("nan")}}, "user/project", "model", retry=2)
assert bool(result) == ({status} == 200)
assert (platform._api_key is None) == ({status} == 401)
if result:
...
""",
    ...
    assert captured[0] == (
        ...
        {
            "event": "epoch_end",
            "data": {"epoch": 0, "loss": None},
            "project": "user/project",
            "name": "model",
        },
    )
    assert len(captured) == (2 if status in {408, 429, 500} else 1)
