...
class Predictor(BasePredictor):
    ...
    def generate(
        ...
        for crop_region, layer_idx in zip(crop_regions, layer_idxs):
            ...
            region_areas.append(area.expand(crop_masks.size(0)))
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        names = dict(enumerate(str(i) for i in range(pred_masks.size(0))))
        ...
        for masks, orig_img, img_path in zip([pred_masks], orig_imgs, self.batch[0]):
            if masks.size(0) == 0:
                ...
                cls = torch.arange(pred_masks.size(0), dtype=torch.int32, device=pred_masks.device)
...
class SAM2VideoPredictor(SAM2Predictor):
    ...
    def inference(self, im, bboxes=None, points=None, labels=None, masks=None):
        ...
        return pred_masks, torch.ones(pred_masks.size(0), dtype=pred_masks.dtype, device=pred_masks.device)
