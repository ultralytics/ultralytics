...
class DFL(nn.Module):
    ...
    def __init__(self, c1: int = 16):
        ...
        """
        super().__init__()
        self.register_buffer("proj", torch.arange(c1, dtype=torch.float32).view(1, c1, 1))
        self.c1 = c1
    ...
    def __setstate__(self, state):
        ...
        if "proj" not in self._buffers:
            self._buffers["proj"] = self.conv.weight.data.view(1, -1, 1)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply the DFL module to input tensor and return transformed output."""
        b, _, a = x.shape
        # Shape: (b, 4, c1, a) -> softmax over c1 dim -> weighted sum
        x = x.view(b, 4, self.c1, a).softmax(2)  # (b, 4, c1, a)
        return (x * self.proj).sum(2)  # (b, 4, a)
