import argparse, os, urllib.request, numpy as np, time, threading, queue
    ...
    def process_frames(self):
        ...
        for i, (x1, y1, x2, y2) in enumerate(db):
            if i < len(dc):
                ...
                if i < len(tr):
                    vid = tr[i]
                    cv2.putText(display_frame, f"ID:{vid}", (int(x1), int(y1)-10), 
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="Simple Drone Detection with GStreamer Overlay & Tkinter GUI"
    )
    p.add_argument("video", help="Video file or RTSP URL")
    p.add_argument("--model",     default="yolo11m-visdrone.pt", help="YOLO11m PyTorch weights file")
    p.add_argument("--engine",    default="yolo11m-visdrone-640.engine", help="YOLO11m TensorRT engine file")
    p.add_argument("--model-url", default="https://github.com/ultralytics/assets/releases/download/v8.3.0/yolo11m.pt",
    ...
    if os.path.exists(args.engine):
        ...
        model.export(format="engine", name=os.path.basename(args.engine).split('.')[0], save_dir=os.path.dirname(args.engine), imgsz=args.imgsz)
        model = YOLO(args.engine)
    else:
