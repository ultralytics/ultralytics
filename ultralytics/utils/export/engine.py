...
def onnx2engine(
    ...
    if dynamic:
        ...
        for inp in inputs:
            # An axis the parsed ONNX graph reports as static (not -1) must keep that exact value in the
            # profile: TensorRT rejects a range for an axis the network doesn't actually declare dynamic
            # (e.g. NMS-embedded exports, where only batch stays dynamic and height/width are traced fixed).
