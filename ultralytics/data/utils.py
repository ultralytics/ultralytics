    ...
    def get_json(self, save: bool = False, verbose: bool = False) -> dict:
        ...
        for split in "train", "val", "test":
            ...
            if self.task == "classify":
                ...
                cls = HumanDataset if self.task == "human" else YOLODataset
                dataset = cls(img_path=self.data[split], data=self.data, task=self.task)
                x = np.array(
                    [
    ...
    def process_images(self) -> Path:
        ...
        for split in "train", "val", "test":
            ...
            if self.task == "classify":
                ...
                cls = HumanDataset if self.task == "human" else YOLODataset
                dataset = cls(img_path=split_path, data=self.data, task=self.task)
