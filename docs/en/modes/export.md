...
QAT fine-tunes an already-trained checkpoint, so it needs a small learning rate: the training defaults are built for long runs from scratch, and at `lr0=0.0002` the fine-tune lands _below_ post-training quantization, while `lr0=0.00001` recovers a good part of what INT8 costs. On `yolo26n` the recovery saturates by about five epochs; the larger sizes are still improving at that point, so budget more epochs for them.

On COCO, `yolo26n` gives up 0.81 mAP50-95 to INT8 after five epochs of QAT against 1.18 for post-training quantization of the same graph, and `yolo26s` 1.25 against 1.44. Accuracy climbs monotonically once QAT is running; what costs accuracy is the first epoch, where restarting optimization moves the checkpoint off the minimum it converged to, and that hit grows with model size — negligible on `yolo26n`, 1.0 mAP50-95 on `yolo26m`, 1.5 on `yolo26x`. Fine-tuning causes it rather than quantization: `yolo26m` drops 1.6 in its first epoch with quantization switched off entirely. Five epochs win back 0.4 to 0.9, which is enough to beat a plain INT8 export on `yolo26n` and `yolo26s` but not from `yolo26m` up, where post-training quantization still scores higher at this budget. Lowering `lr0` shrinks the first-epoch hit and the recovery in equal measure (`0.000002` is a wash), so spend epochs rather than tuning the learning rate, and validate against post-training quantization for your own model and size. Budgeting epochs generously is safe: `best.pt` comes from the best-scoring epoch and `patience` ends the run once the score stops improving, so an over-long schedule costs time rather than accuracy.
