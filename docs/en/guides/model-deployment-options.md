...
- **[Core AI](../integrations/coreai.md)** (`coreai`): Apple's new `.aimodel` format for iOS 27 and macOS 27, scheduled across the CPU, GPU, and Apple Neural Engine; export needs macOS 26 or later on Apple silicon.
...
| Core AI           | Optimized for Apple silicon                     | iOS 27 and macOS 27; export needs macOS 26+    | Apple framework; not in iOS/Flutter SDKs yet  | On-device ML on next-gen Apple platforms    | Tied to Apple OS releases; currently beta      | On-device inference keeps data local              | Apple Neural Engine, GPU, and CPU   |
