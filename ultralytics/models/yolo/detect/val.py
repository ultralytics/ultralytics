        ...
        if self.args.save_json and (self.is_coco or self.is_lvis) and len(self.jdict):
            ...
            try:
                ...
                for i, iou_type in enumerate(iou_types):
                    ...
                    # record mAP for small, medium, large objects as well (COCO keypoints has no small area range)
                    for area in "small", "medium", "large":
                        if f"AP_{area}" in val.stats_as_dict:
                            stats[f"metrics/mAP_{area}({suffix[i][0]})"] = val.stats_as_dict[f"AP_{area}"]
                    # update fitness
