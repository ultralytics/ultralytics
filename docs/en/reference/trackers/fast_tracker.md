...
!!! abstract "Summary"
    ...
    === "<span class="doc-kind doc-kind-class">Classes</span>"
        ...
        - [`FastSTrack`](#ultralytics.trackers.fast_tracker.FastSTrack)
        - [`FASTTracker`](#ultralytics.trackers.fast_tracker.FASTTracker)

    === "<span class="doc-kind doc-kind-method">Methods</span>"

        - [`FastSTrack._push_history`](#ultralytics.trackers.fast_tracker.FastSTrack._push_history)
        - [`FastSTrack.activate`](#ultralytics.trackers.fast_tracker.FastSTrack.activate)
        - [`FastSTrack.re_activate`](#ultralytics.trackers.fast_tracker.FastSTrack.re_activate)
        - [`FastSTrack.update`](#ultralytics.trackers.fast_tracker.FastSTrack.update)
        - [`FASTTracker.init_track`](#ultralytics.trackers.fast_tracker.FASTTracker.init_track)
        - [`FASTTracker._apply_match`](#ultralytics.trackers.fast_tracker.FASTTracker._apply_match)
        - [`FASTTracker._second_association`](#ultralytics.trackers.fast_tracker.FASTTracker._second_association)
        - [`FASTTracker._init_new_tracks`](#ultralytics.trackers.fast_tracker.FASTTracker._init_new_tracks)
        - [`FASTTracker._remove_stale_lost`](#ultralytics.trackers.fast_tracker.FASTTracker._remove_stale_lost)
        - [`FASTTracker._format_output`](#ultralytics.trackers.fast_tracker.FASTTracker._format_output)
        - [`FASTTracker._handle_occlusions`](#ultralytics.trackers.fast_tracker.FASTTracker._handle_occlusions)


## Class `ultralytics.trackers.fast_tracker.FastSTrack` {#ultralytics.trackers.fast\_tracker.FastSTrack}

```python
FastSTrack(xywh: np.ndarray, score: float, cls: Any, history_len: int = 16)
```
...
Extends `STrack` with a bounded ring-buffer of recent Kalman means and per-track occlusion bookkeeping. The history buffer enables rolling the Kalman state back to a pre-occlusion frame when a neighbor suddenly covers the target. The buffer is a fixed-size `collections.deque`, so memory stays bounded regardless of track lifetime.
...
| `history_len` | `int` | Maximum number of past Kalman mean vectors kept for occlusion rollback. | `16` |

**Attributes**

| Name | Type | Description |
| --- | --- | --- |
| `mean_history` | `collections.deque` | Bounded ring-buffer of recent ``(mean, covariance)`` snapshots, newest last, capped at ``history_len`` entries. Rolling back both together keeps the Kalman state internally consistent when a track is restored after an occlusion gap. |
...
| `was_recently_occluded` | `bool` | Sticky flag kept for ``occ_reappear_window`` frames, used by `FASTTracker` to extend the re-find window for tracks that went lost while occluded. |

**Methods**

| Name | Description |
| --- | --- |
| [`_push_history`](#ultralytics.trackers.fast_tracker.FastSTrack._push_history) | Append `(mean, covariance)` copies of the current Kalman state to the bounded buffer. |
| [`activate`](#ultralytics.trackers.fast_tracker.FastSTrack.activate) | Activate the track and seed its mean history. |
| [`re_activate`](#ultralytics.trackers.fast_tracker.FastSTrack.re_activate) | Re-activate a previously lost track and clear any stale occlusion bookkeeping. |
| [`update`](#ultralytics.trackers.fast_tracker.FastSTrack.update) | Update the track with a newly matched detection. |
...
<details>
<summary>Source code in <code>ultralytics/trackers/fast_tracker.py</code></summary>

<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/trackers/fast_tracker.py#L18-L101">View on GitHub</a>
```python
...
### Method `ultralytics.trackers.fast_tracker.FastSTrack._push_history` {#ultralytics.trackers.fast\_tracker.FastSTrack.\_push\_history}
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/trackers/fast_tracker.py#L62-L65">View on GitHub</a>
```python
def _push_history(self):
    """Append `(mean, covariance)` copies of the current Kalman state to the bounded buffer."""
...
### Method `ultralytics.trackers.fast_tracker.FastSTrack.activate` {#ultralytics.trackers.fast\_tracker.FastSTrack.activate}
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/trackers/fast_tracker.py#L67-L75">View on GitHub</a>
```python
def activate(self, kalman_filter, frame_id: int):
    """Activate the track and seed its mean history.
...
### Method `ultralytics.trackers.fast_tracker.FastSTrack.re_activate` {#ultralytics.trackers.fast\_tracker.FastSTrack.re\_activate}
...
| Name | Type | Description | Default |
| --- | --- | --- | --- |
| `new_track` | `FastSTrack` | Detection used to revive this track. | *required* |
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/trackers/fast_tracker.py#L77-L91">View on GitHub</a>
```python
def re_activate(self, new_track, frame_id: int, new_id: bool = False):
    """Re-activate a previously lost track and clear any stale occlusion bookkeeping.
...
### Method `ultralytics.trackers.fast_tracker.FastSTrack.update` {#ultralytics.trackers.fast\_tracker.FastSTrack.update}
...
| Name | Type | Description | Default |
| --- | --- | --- | --- |
| `new_track` | `FastSTrack` | Matched detection for this frame. | *required* |
| `frame_id` | `int` | Current frame id. | *required* |

<details>
<summary>Source code in <code>ultralytics/trackers/fast_tracker.py</code></summary>

<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/trackers/fast_tracker.py#L93-L101">View on GitHub</a>
```python
def update(self, new_track, frame_id: int):
    """Update the track with a newly matched detection.
...
## Class `ultralytics.trackers.fast_tracker.FASTTracker` {#ultralytics.trackers.fast\_tracker.FASTTracker}
...
Adapted from the reference implementation in the FastTracker paper (arXiv:2508.14370). FastTracker extends `BYTETracker` with lightweight mechanisms that reduce ID switches through crowd occlusions without sacrificing throughput. Unmatched tracks whose area is strongly covered by an active neighbor are flagged as occluded and their Kalman state is rolled back to a pre-occlusion frame, with a one-shot bbox enlargement and dampened motion so they survive the occlusion. An occluded track is kept alive for an extra grace window before being marked lost, and once lost it stays re-findable for an extended window beyond the regular ``track_buffer``. New detections that strongly overlap an already-active track are suppressed at spawn time to prevent ghost IDs.

All added work uses vectorized IoU / coverage matrices and only runs on unmatched tracks, so the per-frame overhead over `BYTETracker` stays on the order of a few hundred microseconds.

Any FastTracker-specific key missing on ``args`` falls back to a sensible default, so FastTracker can also be driven by a plain ByteTrack config.

**Args**

| Name | Type | Description | Default |
| --- | --- | --- | --- |
| `args` | `Namespace \| IterableSimpleNamespace` | Parsed tracker config. Must provide the BYTETracker keys (``track_high_thresh``, ``track_low_thresh``, ``new_track_thresh``, ``track_buffer``, ``match_thresh``, ``fuse_score``) and may provide the FastTracker-specific keys described in the class docstring. | *required* |

**Attributes**

| Name | Type | Description |
| --- | --- | --- |
| `reset_velocity_offset_occ` | `int` | Number of frames to look back when restoring Kalman velocity at occlusion onset. |
...
| `init_iou_suppress` | `float` | IoU threshold above which a new detection is prevented from spawning a fresh track. Set to 1.0 to disable suppression. |
| `occ_cover_thresh` | `float` | Fraction of a track's area that must be covered by another active track to declare occlusion. |
| `occ_reappear_window` | `int` | Frames a recently-occluded lost track stays re-findable beyond the regular ``track_buffer``. |

**Methods**

| Name | Description |
| --- | --- |
| [`_apply_match`](#ultralytics.trackers.fast_tracker.FASTTracker._apply_match) | Update or re-activate a track and clear any occlusion bookkeeping on a successful match. |
| [`_format_output`](#ultralytics.trackers.fast_tracker.FASTTracker._format_output) | Only emit tracks updated this frame to avoid stale ``idx`` values. |
| [`_handle_occlusions`](#ultralytics.trackers.fast_tracker.FASTTracker._handle_occlusions) | Flag unmatched tracked tracks as occluded when covered by an active neighbor. |
| [`_init_new_tracks`](#ultralytics.trackers.fast_tracker.FASTTracker._init_new_tracks) | Activate new tracks, suppressing detections that heavily overlap already-active tracks. |
| [`_remove_stale_lost`](#ultralytics.trackers.fast_tracker.FASTTracker._remove_stale_lost) | Remove lost tracks, with a grace window for recently-occluded ones. |
| [`_second_association`](#ultralytics.trackers.fast_tracker.FASTTracker._second_association) | Second-stage association + occlusion handling (replaces base mark-lost loop). |
| [`init_track`](#ultralytics.trackers.fast_tracker.FASTTracker.init_track) | Build `FastSTrack` instances from a ``Results``-like object. |
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/trackers/fast_tracker.py#L104-L350">View on GitHub</a>
```python
...
### Method `ultralytics.trackers.fast_tracker.FASTTracker._apply_match` {#ultralytics.trackers.fast\_tracker.FASTTracker.\_apply\_match}
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/trackers/fast_tracker.py#L195-L200">View on GitHub</a>
```python
def _apply_match(self, track: STrack, det: STrack, activated: list[STrack], refind: list[STrack]) -> None:
    """Update or re-activate a track and clear any occlusion bookkeeping on a successful match."""
...
### Method `ultralytics.trackers.fast_tracker.FASTTracker._format_output` {#ultralytics.trackers.fast\_tracker.FASTTracker.\_format\_output}
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/trackers/fast_tracker.py#L261-L266">View on GitHub</a>
```python
def _format_output(self) -> np.ndarray:
    """Only emit tracks updated this frame to avoid stale ``idx`` values."""
    return np.asarray(
...
### Method `ultralytics.trackers.fast_tracker.FASTTracker._handle_occlusions` {#ultralytics.trackers.fast\_tracker.FASTTracker.\_handle\_occlusions}
...
For each unmatched track, computes the fraction of its area covered by any currently-active track. If coverage exceeds ``occ_cover_thresh``, the track is marked occluded, its Kalman state is rolled back using the ring-buffer history (velocity from ``reset_velocity_offset_occ`` frames ago, position from ``reset_pos_offset_occ`` frames ago), the bbox height is scaled by ``enlarge_bbox_occ`` once, and velocity is dampened by ``dampen_motion_occ``. Tracks that remain unmatched for more than two frames are transitioned to Lost, except while they are within the ``active_occ_to_lost_thresh`` occluded-grace window.

**Args**

| Name | Type | Description | Default |
| --- | --- | --- | --- |
| `r_tracked` | `list[FastSTrack]` | Candidate track pool. | *required* |
| `u_track` | `list[int] \| np.ndarray` | Indices into ``r_tracked`` of tracks still unmatched. | *required* |
| `activated_stracks` | `list[FastSTrack]` | Tracks already matched this frame; used as the pool of potential occluders. | *required* |
| `lost_stracks` | `list[FastSTrack]` | Output list; tracks transitioned to Lost are appended. | *required* |

<details>
<summary>Source code in <code>ultralytics/trackers/fast_tracker.py</code></summary>

<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/trackers/fast_tracker.py#L268-L350">View on GitHub</a>
```python
def _handle_occlusions(self, r_tracked, u_track, activated_stracks, lost_stracks):
    """Flag unmatched tracked tracks as occluded when covered by an active neighbor.
...
### Method `ultralytics.trackers.fast_tracker.FASTTracker._init_new_tracks` {#ultralytics.trackers.fast\_tracker.FASTTracker.\_init\_new\_tracks}
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/trackers/fast_tracker.py#L221-L249">View on GitHub</a>
```python
def _init_new_tracks(
    self,
    u_detection: list[int],
...
### Method `ultralytics.trackers.fast_tracker.FASTTracker._remove_stale_lost` {#ultralytics.trackers.fast\_tracker.FASTTracker.\_remove\_stale\_lost}
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/trackers/fast_tracker.py#L251-L259">View on GitHub</a>
```python
def _remove_stale_lost(self, removed: list[STrack]) -> None:
    """Remove lost tracks, with a grace window for recently-occluded ones."""
...
### Method `ultralytics.trackers.fast_tracker.FASTTracker._second_association` {#ultralytics.trackers.fast\_tracker.FASTTracker.\_second\_association}
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/trackers/fast_tracker.py#L202-L219">View on GitHub</a>
```python
def _second_association(
    self,
    strack_pool: list[STrack],
...
### Method `ultralytics.trackers.fast_tracker.FASTTracker.init_track` {#ultralytics.trackers.fast\_tracker.FASTTracker.init\_track}
...
| `img` | `np.ndarray \| None` | Current BGR frame. Unused by FastTracker; accepted for signature parity with other trackers. | `None` |
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/trackers/fast_tracker.py#L176-L193">View on GitHub</a>
```python
def init_track(self, results, img: np.ndarray | None = None) -> list[FastSTrack]:
    """Build `FastSTrack` instances from a ``Results``-like object.
