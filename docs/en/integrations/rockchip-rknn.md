...
| `quantize` | `int` or `str`   | `16`       | Quantization precision: `16` (FP16) builds the default floating-point RKNN model for FP16-capable targets; `8` (INT8) builds an INT8-quantized model and is required for INT8-only targets such as RV1103 and RV1106. RKNN export has no separate FP32 mode. Replaces the deprecated `half`/`int8` flags. |
