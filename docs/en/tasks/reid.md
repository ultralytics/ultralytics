...
| `reid_arcface`    | `bool`  | `False` | When `True`, the identity cross-entropy uses a **sub-center ArcFace** angular margin instead of plain softmax (triplet stays active). Gives sharper separation of near-identical classes, suited to fine-grained catalogs such as product SKUs. |

!!! tip "Fine-grained catalogs (product SKUs, package variants)"

    For classes that look nearly identical, set `reid_arcface=True`. Introduced for face recognition, sub-center ArcFace places the margin between each sample and its class center ([Deng et al., CVPR 2019](https://openaccess.thecvf.com/content_CVPR_2019/html/Deng_ArcFace_Additive_Angular_Margin_Loss_for_Deep_Face_Recognition_CVPR_2019_paper.html); sub-center variant, [Deng et al., ECCV 2020](https://www.ecva.net/papers/eccv_2020/papers_ECCV/papers/123560715.pdf)) rather than between sampled pairs as triplet does. The K sub-centers per class add robustness to intra-class variation. This angular margin transfers well to fine-grained retrieval such as product SKUs, though the published comparisons are on face benchmarks.
