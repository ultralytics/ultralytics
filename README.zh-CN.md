...
- **Ultralytics 企业许可证**：专为开发和生产用途设计，此许可证涵盖 AI 应用的构建与部署。它允许将 Ultralytics 软件和 AI 模型无缝集成到实际业务产品和服务中，包括内部工具、自动化工作流和生产部署，绕过 AGPL-3.0 的开源要求。如果您的使用场景涉及在任何生产或业务环境中开发或部署模型，请通过 [Ultralytics 授权许可](https://www.ultralytics.com/license)与我们联系。
