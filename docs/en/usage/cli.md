...
!!! example
    ...
    === "Syntax"
        ...
        Where:
        - `TASK` (optional) is one of [detect, segment, classify, pose, obb, semantic]
...
- `TASK` (optional) is one of `[detect, segment, classify, pose, obb, semantic]`. If not explicitly passed, YOLO will attempt to infer the `TASK` from the model type.
- `MODE` (required) is one of `[train, val, predict, export, track, benchmark]`
...
The Ultralytics YOLO CLI supports various tasks, including [detection](../tasks/detect.md), [segmentation](../tasks/segment.md), [classification](../tasks/classify.md), [pose estimation](../tasks/pose.md), [oriented bounding box detection](../tasks/obb.md), and [semantic segmentation](../tasks/semantic.md). You can also perform operations like:
