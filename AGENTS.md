...
4. **Keep scope and tests minimal**: Implement only the simplest complete solution. Avoid impossible-state handling, speculative flags, compatibility shims, policy scaffolding, and unrelated cleanup. Add only the smallest targeted regression test needed to prove the change; never add redundant coverage or broad test scaffolding.
