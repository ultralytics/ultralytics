...
!!! example
    ...
    === "Syntax"
        ...
        Where:
        - `TASK` (optional) is one of [detect, segment, classify, pose, obb, semseg]
...
- `TASK` (optional) is one of `[detect, segment, classify, pose, obb, semseg]`. If not explicitly passed, YOLO will attempt to infer the `TASK` from the model type.
- `MODE` (required) is one of `[train, val, predict, export, track, benchmark]`
