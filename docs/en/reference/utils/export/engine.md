...
!!! abstract "Summary"
    ...
    === "<span class="doc-kind doc-kind-class">Classes</span>"
        ...
        - [`_NormalizeCoords`](#ultralytics.utils.export.engine._NormalizeCoords)

    === "<span class="doc-kind doc-kind-method">Methods</span>"

        - [`_NormalizeCoords.forward`](#ultralytics.utils.export.engine._NormalizeCoords.forward)

    === "<span class="doc-kind doc-kind-function">Functions</span>"

        - [`best_onnx_opset`](#ultralytics.utils.export.engine.best_onnx_opset)
        - [`torch2onnx`](#ultralytics.utils.export.engine.torch2onnx)
        - [`modelopt_quantize_onnx`](#ultralytics.utils.export.engine.modelopt_quantize_onnx)
        - [`onnx2engine`](#ultralytics.utils.export.engine.onnx2engine)


## Class `ultralytics.utils.export.engine._NormalizeCoords` {#ultralytics.utils.export.engine.\_NormalizeCoords}

```python
_NormalizeCoords(model: torch.nn.Module, h: int, w: int, task: str, nc: int, kpt_shape: tuple | None)
```
...
| `task` | `str` |  | *required* |
| `nc` | `int` |  | *required* |
| `kpt_shape` | `tuple \| None` |  | *required* |

**Methods**

| Name | Description |
| --- | --- |
| [`forward`](#ultralytics.utils.export.engine._NormalizeCoords.forward) | Run the wrapped model and normalize its coordinate channels by input size. |

<details>
<summary>Source code in <code>ultralytics/utils/export/engine.py</code></summary>

<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/export/engine.py#L16-L47">View on GitHub</a>
```python
...
### Method `ultralytics.utils.export.engine._NormalizeCoords.forward` {#ultralytics.utils.export.engine.\_NormalizeCoords.forward}
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/export/engine.py#L29-L47">View on GitHub</a>
```python
def forward(self, x: torch.Tensor):
    """Run the wrapped model and normalize its coordinate channels by input size."""
    y = self.model(x)
...
## Function `ultralytics.utils.export.engine.best_onnx_opset` {#ultralytics.utils.export.engine.best\_onnx\_opset}
...
| Name | Type | Description | Default |
| --- | --- | --- | --- |
| `onnx` | `types.ModuleType` |  | *required* |
| `cuda` | `bool` |  | `False` |
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/export/engine.py#L50-L79">View on GitHub</a>
```python
def best_onnx_opset(onnx: types.ModuleType, cuda: bool = False, quantize: int | str | None = None) -> int:
    """Return max ONNX opset for this torch version with ONNX fallback."""
...
## Function `ultralytics.utils.export.engine.torch2onnx` {#ultralytics.utils.export.engine.torch2onnx}
...
| `im` | `torch.Tensor \| tuple[torch.Tensor, ...]` | Example input tensor(s) for tracing. | *required* |
| `output_file` | `Path \| str` | Path to save the exported ONNX file. | *required* |
| `opset` | `int` | ONNX opset version to use for export. | `14` |
| `input_names` | `list[str] \| None` | List of input tensor names. Defaults to ``["images"]``. | `None` |
| `output_names` | `list[str] \| None` | List of output tensor names. Defaults to ``["output0"]``. | `None` |
| `dynamic` | `dict \| None` | Dictionary specifying dynamic axes for inputs and outputs. | `None` |
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/export/engine.py#L83-L126">View on GitHub</a>
```python
@ThreadingLocked()
def torch2onnx(
    model: torch.nn.Module,
...
## Function `ultralytics.utils.export.engine.modelopt_quantize_onnx` {#ultralytics.utils.export.engine.modelopt\_quantize\_onnx}
...
TensorRT 11 is strongly-typed only: it removed the FP16/INT8 builder flags and the ``IInt8Calibrator`` interface, so reduced precision must be expressed in the ONNX graph itself before building. FP16 is applied via ModelOpt AutoCast mixed-precision conversion and INT8 via explicit Q/DQ quantization with calibration.

**Args**

| Name | Type | Description | Default |
| --- | --- | --- | --- |
| `onnx_file` | `str` | Path to the FP32 ONNX file to convert. | *required* |
| `quantize` | `int \| str \| None` | Precision scheme, 8 for INT8 Q/DQ nodes or 16 for FP16 precision. | `None` |
| `dataset` | `ultralytics.data.build.InfiniteDataLoader \| None` | Dataloader providing INT8 calibration images. Required when ``quantize=8``. | `None` |
| `shape` | `tuple[int, int, int, int]` | Input shape (batch, channels, height, width) used for dynamic calibration. | `(1, 3, 640, 640)` |
| `dynamic` | `bool` | Whether the ONNX model uses dynamic input shapes. | `False` |
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/export/engine.py#L129-L208">View on GitHub</a>
```python
def modelopt_quantize_onnx(
    onnx_file: str,
    quantize: int | str | None = None,
...
## Function `ultralytics.utils.export.engine.onnx2engine` {#ultralytics.utils.export.engine.onnx2engine}
...
| Name | Type | Description | Default |
| --- | --- | --- | --- |
| `onnx_file` | `str` | Path to the ONNX file to be converted. | *required* |
| `output_file` | `Path \| str \| None` | Path to save the generated TensorRT engine file. | `None` |
| `workspace` | `int \| None` | Workspace size in GB for TensorRT. | `None` |
| `quantize` | `int \| str \| None` | Precision scheme, 16 for FP16 or 8 for INT8. | `None` |
| `dynamic` | `bool, optional` | Enable dynamic input shapes. | `False` |
| `shape` | `tuple[int, int, int, int], optional` | Input shape (batch, channels, height, width). | `(1, 3, 640, 640)` |
| `dla` | `int \| None` | DLA core to use (Jetson devices only). | `None` |
| `dataset` | `ultralytics.data.build.InfiniteDataLoader, optional` | Dataset for INT8 calibration. | `None` |
| `metadata` | `dict \| None` | Metadata to include in the engine file. | `None` |
| `verbose` | `bool, optional` | Enable verbose logging. | `False` |
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/export/engine.py#L211-L457">View on GitHub</a>
```python
def onnx2engine(
    onnx_file: str,
    output_file: Path | str | None = None,
