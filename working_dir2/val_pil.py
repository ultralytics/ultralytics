#!/usr/bin/env python3
"""Validate an RF-DETR engine with PIL bilinear + antialias preprocessing.

RF-DETR was trained with torchvision.transforms.functional.resize (BILINEAR +
antialias=True).  Ultralytics' BaseDataset.load_image uses cv2.INTER_LINEAR
(no antialias), causing a ~0.4 mAP regression.  This script patches load_image
to match training-time preprocessing.

Monkey-patches applied at runtime — val.py is NOT modified:
  1. PIL bilinear + antialias resize  (RTDETRDataset.load_image)
  2. coco91 → coco80 class remapping  (RTDETRValidator.postprocess)
  3. stride=1 imgsz fix               (RTDETRValidator.__init__)

Usage:
  python working_dir2/val_rfdetr_pil.py \\
...
# ── Patch 1: PIL bilinear + antialias load_image ──────────────────────────────
# Returns BGR so that Format._format_img's unconditional channel flip ([::-1])
# converts it back to RGB for the model.
def _pil_antialias_load_image(self, i, rect_mode=False):
    image = PILImage.open(self.im_files[i]).convert("RGB")
    hw0 = (image.height, image.width)
    pil_resized = TF.resize(image, [self.imgsz, self.imgsz])  # bilinear + antialias
    im = np.array(pil_resized)[:, :, ::-1].copy()             # RGB → BGR
    return im, hw0, (self.imgsz, self.imgsz)

RTDETRDataset.load_image = _pil_antialias_load_image
print("[val_rfdetr_pil] Patch 1: PIL bilinear+antialias load_image applied")

# ── Patch 2: coco91 → coco80 class remapping ─────────────────────────────────
...
print("[val_rfdetr_pil] Patch 2: coco91 remapping applied")

# ── Patch 3: stride=1 imgsz fix ──────────────────────────────────────────────
...
print("[val_rfdetr_pil] Patch 3: stride=1 imgsz fix applied (init + warmup)")
print("[val_rfdetr_pil] Preprocessing: PIL bilinear + antialias (matches RF-DETR training)")
...
def main():
    ...
    if imgsz is None and args.model.endswith(".engine"):
        ...
        print(f"[val_rfdetr_pil] Auto-detected engine resolution: {imgsz}x{imgsz}")
