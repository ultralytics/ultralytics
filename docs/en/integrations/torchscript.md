...
| `dynamic`  | `bool`           | `False`         | Allows dynamic input sizes, enhancing flexibility in handling varying image dimensions. Known issue with `nms=True`: filtering silently stops applying at sizes other than the traced one. |
