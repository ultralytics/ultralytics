...
class exportPaddleTask(Thread):
    ...
    def __init__(self, rds: Redis, taskId: str, taskName: str, taskNameList: list, taskProcDict: dict, modelId: str, call_back):
        super().__init__()
        self.daemon = True
    ...
    def export(self):
        self.rds.hset(f'{self.taskId}_{self.taskName}_export_progress', mapping={
                      'status': 1, 'epoch': 0})
        log_file = open(
            f'./log/export_log_{self.taskId}_{self.taskName}.txt', 'w', encoding='utf-8')
        train_command = [
            "source activate paddle && python export.py"]
        ...
        if not psutil.pid_exists(process.pid):  # 后台程序出错
            ...
            self.rds.hset(f'{self.taskId}_{self.taskName}_export_progress',
            ...
            if not psutil.pid_exists(process.pid):  # 后台进程已结束
                ...
                self.rds.xadd(export_result_topic_name, {
                    'result': f'当前导出任务:{self.taskId}_{self.taskName}正常结束'}, maxlen=100)
            else:
                # 给Redis发消息,出现异常后台进程没有正确结束
                self.rds.xadd(export_result_topic_name, {
                    'result': f'当前导出任务:{self.taskId}_{self.taskName}异常退出'}, maxlen=100)
