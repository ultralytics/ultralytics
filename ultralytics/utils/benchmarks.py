...
from ultralytics.utils import ASSETS, LINUX, MACOS, ARM64, WINDOWS, LOGGER, TQDM, WEIGHTS_DIR
...
def benchmark(
    ...
    for i, (name, format, suffix, cpu, gpu) in export_formats().iterrows():  # index, (name, format, suffix, CPU, GPU)
        ...
        try:
            # Checks
            if i == 9:  # Edge TPU
                assert not (ARM64 or MACOS or WINDOWS), "Edge TPU export only supported on Linux"
            ...
            if i in {5}:  # CoreML
                assert MACOS or not ARM64, "export not supported on ARM64 Raspberry Pi"
