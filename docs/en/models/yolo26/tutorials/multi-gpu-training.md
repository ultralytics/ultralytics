---
comments: true
description: Train Ultralytics YOLO26 across multiple GPUs using PyTorch DDP. Learn device arguments, batch scaling, and best practices for distributed training.
keywords: YOLO26, multi-GPU, DDP, distributed training, PyTorch, Ultralytics, DistributedDataParallel, NCCL
canonical: https://docs.ultralytics.com/models/yolo26/tutorials/multi-gpu-training/
---
...
<script type="application/ld+json">
{
  ...
  "@type": "TechArticle",
  "headline": "Multi-GPU Training with YOLO26 Using PyTorch DDP",
  "description": "Train Ultralytics YOLO26 across multiple GPUs using PyTorch DDP. Learn device arguments, batch scaling, and best practices for distributed training.",
  "url": "https://docs.ultralytics.com/models/yolo26/tutorials/multi-gpu-training/",
  ...
  "mainEntityOfPage": "https://docs.ultralytics.com/models/yolo26/tutorials/multi-gpu-training/"
}
</script>

# Multi-GPU Training with YOLO26

<!-- NOTE FOR MURAT: Please verify that DDP training works correctly with YOLO26's end-to-end NMS-free architecture (the E2E head may require special handling in DDP mode). Add measured scaling efficiency numbers (e.g. 1x, 1.85x, 3.6x throughput for 1/2/4 GPUs). Confirm `torchrun` and the `device` argument syntax work as documented below. Add any known issues or limitations with specific GPU counts or server configurations. -->

[Ultralytics YOLO26](../../../models/yolo26.md) supports multi-GPU training via PyTorch [DistributedDataParallel (DDP)](https://pytorch.org/docs/stable/notes/ddp.html), which distributes the workload evenly across all available GPUs on a single machine. This is the fastest way to cut wall-clock training time when you have access to multiple NVIDIA GPUs.
...
- Ultralytics package: `pip install "ultralytics>=26.0.0"`

## Single-GPU vs Multi-GPU Command Comparison

| Setup | CLI command | Notes |
|---|---|---|
| Single GPU 0 | `yolo train model=yolo26n.pt device=0` | Default single-GPU |
| Single GPU 1 | `yolo train model=yolo26n.pt device=1` | Use second GPU |
| 2 GPUs (0 and 1) | `yolo train model=yolo26n.pt device=0,1` | DDP auto-launched |
| 4 GPUs (0–3) | `yolo train model=yolo26n.pt device=0,1,2,3` | 4-GPU DDP |
| All GPUs | `yolo train model=yolo26n.pt device=0,1,2,3` | List all GPU IDs |
...
=== "CLI"
    ...
    yolo train model=yolo26n.pt data=coco.yaml epochs=100 batch=32 device=0,1

    # 4-GPU training
    yolo train model=yolo26m.pt data=coco.yaml epochs=100 batch=64 device=0,1,2,3

    # Using torchrun directly (advanced — gives more DDP control)
    torchrun --nproc_per_node 4 -m ultralytics.utils.dist train \
        model=yolo26m.pt data=coco.yaml epochs=100 batch=64
    ```
...
!!! warning "Memory constraints"
    ...
    If you hit CUDA out-of-memory errors, reduce the per-GPU batch size or use a smaller model variant (`yolo26n` vs `yolo26x`).
...
yolo train model=yolo26l.pt data=coco.yaml \
