...
from ultralytics.nn.tasks import (ClassificationModel, DetectionModel, SegmentationModel, attempt_load_one_weight,
                                  guess_model_scale, guess_model_task, nn)
...
class YOLO:
    ...
    def _new(self, cfg: str, task=None, verbose=True):
        ...
        # self.cfg = check_yaml(cfg, hard=False) or check_yaml(cfg)
        # cfg_dict = yaml_load(self.cfg, append_filename=True)  # model dict
        import re
        print('Model requested:', cfg)
        unified_cfg = re.sub(r'(\d+)([nslmx])(.+)?$', r'\1_\3', cfg)
        print('Unified model CFG assumed:', unified_cfg)
        self.cfg = check_yaml(unified_cfg, hard=False) or check_yaml(cfg)
        cfg_dict = yaml_load(self.cfg, append_filename=True)  # model dict
        cfg_dict['scale'] = guess_model_scale(cfg)
