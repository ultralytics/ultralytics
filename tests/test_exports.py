...
def test_hailo_decode_depth_matches_head(cal_a, cal_b):
    ...
    torch.manual_seed(0)
    head = Depth(c_mid=64, ch=(64, 128, 256)).eval()
    head.cal_a.fill_(cal_a)
    head.cal_b.fill_(cal_b)
    logit = {}
    head.head.register_forward_hook(lambda m, i, o: logit.update(v=o))
    feats = [torch.randn(1, 64, 32, 32), torch.randn(1, 128, 16, 16), torch.randn(1, 256, 8, 8)]
    with torch.no_grad():
        reference = head(feats)  # (B, 1, H/4, W/4) calibrated depth map, exactly what `yolo predict` returns

    backend = HailoBackend.__new__(HailoBackend)  # skip HailoRT device init; `_decode_depth` only reads metadata
    backend.metadata = {} if (cal_a, cal_b) == (1.0, 0.0) else {"cal_a": cal_a, "cal_b": cal_b}
    hef_output = logit["v"].permute(0, 2, 3, 1).numpy()  # HEF returns the logit as NHWC (B, H/4, W/4, 1)
    decoded = backend._decode_depth(hef_output)

    assert decoded.shape == reference.shape
    assert torch.allclose(decoded, reference, atol=1e-6)  # host decode is bit-exact vs the PyTorch head


def test_hailo_decode_depth_forward_routing():
    """Hailo `forward` dispatches task='depth' to `_decode_depth` and returns the metric map, not the NMS path."""
    import numpy as np
    ...
    backend.task = "depth"
    backend.metadata = {"cal_a": 0.9, "cal_b": 0.1}
    logit = np.random.randn(1, 8, 8, 1).astype(np.float32)  # single-channel NHWC logit off the chip
    backend.input_info = SimpleNamespace(name="images")
    backend.output_infos = [SimpleNamespace(name="depth_logit")]
    backend.model = SimpleNamespace(infer=lambda feed: {"depth_logit": logit})

    out = backend.forward(torch.zeros(1, 3, 8, 8))

    assert torch.is_tensor(out) and out.shape == (1, 1, 8, 8)  # a depth map, not (B, 300, 6) detections
    assert torch.allclose(out, backend._decode_depth(logit))


def test_hailo_export_depth_metadata(monkeypatch, tmp_path):
    """`export_hailo` cuts at the depth logit conv, keeps a16, and rides `cal_a`/`cal_b` in metadata (DFC mocked)."""
    ...
    class FakeRunner:
        ...
        def translate_onnx_model(self, onnx, name, end_node_names):
            captured["end_nodes"] = end_node_names
        ...
        def compile(self):
            return b"\x00" * 200_000  # >0.1 MB so the try_export size assertion passes

    monkeypatch.setitem(sys.modules, "hailo_sdk_client", SimpleNamespace(ClientRunner=FakeRunner))
    monkeypatch.setitem(sys.modules, "tensorflow", MagicMock())

    head = Depth(c_mid=64, ch=(64, 128, 256))
    head.cal_a.fill_(0.8)
    head.cal_b.fill_(0.15)
    exporter = Exporter.__new__(Exporter)  # bypass __init__; drive export_hailo directly with a stub model
    exporter.args = SimpleNamespace(name="hailo8l", opset=11, end2end=None)
    exporter.imgsz = (640, 640)
    exporter.file = tmp_path / "yolo26n-depth.pt"
    exporter.metadata = {"task": "depth"}
    exporter.model = SimpleNamespace(model=[head], task="depth", names={0: "d"})
    exporter.get_int8_calibration_dataloader = lambda prefix: SimpleNamespace(dataset=[0] * 8)
    exporter.export_onnx = lambda: str(tmp_path / "yolo26n-depth.onnx")

    output_dir = exporter.export_hailo()

    assert captured["end_nodes"] == ["/model.0/head/head.3/Conv"]  # raw logit conv, before clamp/exp/calibration
    assert "precision_mode=a16_w16" in captured["script"]  # a8 collapses the depth map
    meta = YAML.load(Path(output_dir) / "metadata.yaml")
    assert meta["cal_a"] == pytest.approx(0.8) and meta["cal_b"] == pytest.approx(0.15)
    assert meta["nms"] is False
