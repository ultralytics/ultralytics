...
class Dataset(Thread):
    """数据集类--继承Thraed,功能点如下： 支持下载、格式转换、比例划分、路径转移、生成data_yaml文件及数据内容."""
    ...
    def __init__(
        ...
        self.src_dir = [os.path.join(self.prefix_1, item) for item in self.prefix_2]  # 源目录
        self.labels = labels
        self.ratio = ratio
        self.data_yaml_path = data_yaml_path
        self.log = log
        # [datasets/data1,datasets/data2,...]
        self.dst_dir = [os.path.join(self.datasets_dir, item) for item in prefix]  # 本地目录
    ...
    def get_ext_name(self):
        """只读属性,通过@property装饰器实现 外部只能通过Dataset.get_ext_name获取值,而不能直接修改它."""
        return Dataset.__ext_name

    @classmethod  # 静态方法
    def _set_ext_name(cls, new_ext_name):
        """内部使用的方法,允许在一定控制下修改类属性 注意:该方法前面添加了单下划线,表示不建议外部直接调用 正确做法:设置setattr、getattr."""
    ...
    async def download_files(self, obj, idx):
        ...
        return:
            下载后的本地文件路径.
        """
        local_file_path = os.path.join(self.datasets_dir, self.prefix_2[idx], os.path.basename(obj.object_name))
        loop = asyncio.get_event_loop()
        self.log.logger.info(f"开始下载{obj.object_name}到{local_file_path}")
        try:
            # 在线程池中调用同步的fget_object()方法实现异步下载
            await loop.run_in_executor(
                self.executor, self.minio_client.fget_object, self.bucket, obj.object_name, local_file_path
            )
            ...
            for idx, value in enumerate(repeat_download):
                if value:
                    ...
                    if task:
                        ...
                        self.log.logger.info(f"数据集{self.prefix_2[idx]}没有找到符合条件的图片和标签.")
                    task.clear()
        except Exception as ex:
    ...
    def convert_xml2txt(self, xml_path: str, txt_path: str):
        """
        将单个xml文件转换为txt格式 xml文件符合Pascal VOC格式,转换后txt文件的格式为： 每行"类别索引 xmin ymin xmax ymax".
        ...
        try:
            ...
            for obj in root.iter("object"):
                ...
                if name not in self.labels or int(difficult) == 1:
                    self.log.logger.info(f"xml标签转换失败,失败原因:name={name}或者difficult={difficult}")
                    return
                else:
                    # 解析边界框坐标
                    ...
                    line = f"{self.labels.index(name)} " + " ".join([str(item) for item in bb])
                    lines.append(line)
            # 写入txt
    ...
    def generate_yaml(self):
        data_yaml_config = {}
        data_yaml_config["train"] = [os.path.join("../", item, "train") for item in self.dst_dir]
        data_yaml_config["val"] = [os.path.join("../", item, "val") for item in self.dst_dir]
        ...
        with open(self.data_yaml_path, mode="w", encoding="utf-8") as file:
            ...
            yaml.dump(data_yaml_config, file, allow_unicode=True, sort_keys=False)
            file.close()
