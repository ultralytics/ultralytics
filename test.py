...
# 计算相机朝向的旋转矩阵
yaw_rotation_matrix = np.array([[np.cos(yaw), -np.sin(yaw), 0], [np.sin(yaw), np.cos(yaw), 0], [0, 0, 1]])

pitch_rotation_matrix = np.array([[1, 0, 0], [0, np.cos(pitch), -np.sin(pitch)], [0, np.sin(pitch), np.cos(pitch)]])

# 将相机坐标转换为世界坐标系中的坐标
world_coordinates = camera_to_world_coordinates(
    camera_coordinates, camera_position, np.dot(pitch_rotation_matrix, yaw_rotation_matrix)
)
