...
def parse_requirements(file_path=ROOT.parent / "requirements.txt", package=""):
    ...
    Args:
        ...
        package (str, optional): Python package to use instead of requirements.txt file, i.e. package='ultralytics_4bands'.
    ...
    Example:
        ```python
        from ultralytics_4bands.utils.checks import parse_requirements

        parse_requirements(package='ultralytics_4bands')
        ```
    """
...
def check_version(
    ...
    if not current:  # if current is '' or None
        ...
        return True
    elif not current[0].isdigit():  # current is package name rather than version string, i.e. current='ultralytics_4bands'
        try:
...
def check_latest_pypi_version(package_name="ultralytics_4bands"):
    """
...
def check_pip_update_available():
    """
    Checks if a new version of the ultralytics_4bands package is available on PyPI.
        ...
        with contextlib.suppress(Exception):
            ...
            if check_version(__version__, f"<{latest}"):  # check if current version is < latest version
                LOGGER.info(
                    ...
                    f"Update with 'pip install -U ultralytics_4bands'"
                )
                return True
    return False
...
def check_requirements(requirements=ROOT.parent / "requirements.txt", exclude=(), install=True, cmds=""):
    ...
    Example:
        ...
        check_requirements('ultralytics_4bands>=8.0.0')

        # Check multiple packages
        check_requirements(['numpy', 'ultralytics_4bands>=8.0.0'])
        ```
    """
...
def check_yolo(verbose=True, device=""):
    ...
    from ultralytics_4bands.utils.torch_utils import select_device
...
def collect_system_info():
    ...
    from ultralytics_4bands.utils import ENVIRONMENT, is_git_dir
    from ultralytics_4bands.utils.torch_utils import get_cpu_info
    ...
    for r in parse_requirements(package="ultralytics_4bands"):
        try:
...
def check_amp(model):
    ...
    Example:
        ...
        from ultralytics_4bands.utils.checks import check_amp
