...
It is a good choice for longer runs, but set it explicitly rather than relying on `optimizer=auto`. Auto selects **MuSGD** above 10,000 iterations and **AdamW** below that, and it also replaces your `lr0` and `momentum` with its own values (`lr0=0.01`, `momentum=0.9`), so none of the learning rate settings in the tables above take effect. Passing `optimizer=MuSGD` keeps the `lr0` and `momentum` you set. For more on how MuSGD works, see the [training documentation](../modes/train.md#musgd-optimizer).
