...
- `channel_divisor` rounds layer channels up to the nearest multiple (default `8`); set it to `1` to disable rounding for tight embedded channel budgets; `C2fAttn` embedding widths always follow the rounded output channels so attention shapes stay valid.
