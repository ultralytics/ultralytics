...
class ConvEmbed(nn.Module):
    def __init__(self, in_chans=1, embed_dim=96, patch_size=20, stride=10):
        super().__init__()
        self.proj = nn.Conv2d(
    ...
    def forward(self, x):
        return self.proj(x)  # Output: (B, embed_dim, H//stride, W//stride)



class CrossAttention(nn.Module):
    def __init__(self, in_channels, window_size=6, num_heads=4):
        super().__init__()
    ...
    def forward(self, query, key_value):
        ...
        if pad_h > 0 or pad_w > 0:
            query = F.pad(query, (0, pad_w, 0, pad_h))         # pad right and bottom
        ...
        def reverse(x, H_out, W_out):
            ...
            return x[:, :, :H_out, :W_out]  # remove padding
...
class CrossModalAttentionFusion(nn.Module):
    def __init__(self, embed_dim=96):
        ...
        # Cross Attention
        self.cross_attn = CrossAttention(embed_dim, window_size=6, num_heads=4)
        # Global Attention
