...
class LoadNumpyFrames:
    ...
    Attributes:
        path (str): Autogenerated name shared by every frame, so saving writes one video as a file source does.
        im0 (list[np.ndarray]): All frames stored as NumPy arrays.
        mode (str): ``"video"`` — signals video-like iteration to the predictor.
        frame (int): 1-based counter of the most recently yielded frame.
    ...
    Examples:
        ...
        >>> assert len(loader) == 5
        >>> paths, imgs, info = next(iter(loader))
        >>> assert loader.frame == 1  # 1-based
    """

    def __init__(self, frames: list[np.ndarray], channels: int = 3, fps: int = 30):
        """Initialize with a list of BGR uint8 frames.

        Args:
            frames (list[np.ndarray]): Non-empty list of (H, W, C) uint8 NumPy arrays.
            channels (int): Expected number of channels (default 3).
            fps (int): Frames per second for video-saving metadata.
        """
        if not frames:
            raise FileNotFoundError("No frames provided, predict requires at least one frame.")
        self.im0 = [LoadPilAndNumpy._single_check(f, channels) for f in frames]
        self.path = "frames.mp4"  # shared by every frame, otherwise save=True opens one VideoWriter per frame
        self.mode = "video"
        self.frame = 0  # 1-based counter, incremented on each __next__ call
    ...
    def __next__(self) -> tuple[list[str], list[np.ndarray], list[str]]:
        ...
        info = f"video 1/1 (frame {self.frame}/{self.frames}) {self.path}: "
        self.count += 1
        return [self.path], [im0], [info]
...
# Define constants
LOADERS = (LoadStreams, LoadPilAndNumpy, LoadNumpyFrames, LoadImagesAndVideos, LoadScreenshots)
