...
class DetectAux(Detect):
    ...
    "foreground here" logit per anchor, supervised by a BCE loss toward the assigned foreground (see the ``aux_fg``
    hyperparameters). The features are not detached, so the branch injects deep-supervision gradients into the backbone
