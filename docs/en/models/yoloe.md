...
- **Re-parameterizable Region-Text Alignment (RepRTA)** refines text [embeddings](https://www.ultralytics.com/glossary/embeddings) from CLIP through a small auxiliary network that is re-parameterized away after training. Text prompting therefore costs no extra network at inference: the prompt embeddings are simply compared against region features.
...
- **Lazy Region-Prompt Contrast (LRPC)** matches region embeddings against a built-in vocabulary of 4,585 tag names, so prompt-free checkpoints recognize objects with no external prompt and no text encoder.

[Instance segmentation](../tasks/segment.md) comes from a mask branch on the detection head, as in YOLOv8-Seg. Once prompts are set, the open-world modules are re-parameterized into a standard YOLO head, so a prompted YOLOE runs the ordinary detect/segment path.
...
Accuracy, parameters and FLOPs for every scale are in [Performance](#performance).

## Usage Examples

The YOLOE models are easy to integrate into your Python applications. Ultralytics provides a user-friendly [Python API](../usage/python.md) and [CLI commands](../usage/cli.md) to streamline development.

### Train Usage

Fine-tune any released checkpoint on your own YOLO dataset. This mostly follows the [standard YOLO training procedure](../modes/train.md); the difference is that YOLOE needs its own trainer class passed as `trainer`.
...
!!! example
    ...
    === "Fine-Tuning"
        ...
        results = model.train(
            ...
            epochs=80,
            patience=10,
            trainer=YOLOEPESegTrainer,  # <- Important: use the segmentation trainer
        )
        ```
...
!!! tip "Training a detection model instead"
    ...
    Every released checkpoint is a segmentation model. To train a detector, build the model from the matching YAML, load the segmentation weights of the same scale, and swap in the detection trainer. Everything else, including the freeze list above, is unchanged.
...
Text prompting is covered in [Quick Start](#quick-start). The other two modes:
...
!!! example
    ...
    === "Prompt free"
        ...
        Prompt-free checkpoints carry their own vocabulary and behave like an ordinary YOLO model: no prompts, no text encoder, no `set_classes()`. They report objects from a [predefined list of 4,585 classes](https://github.com/xinyu1205/recognize-anything/blob/main/ram/data/ram_tag_list.txt), the tag set used by [Recognize Anything Plus (RAM++)](https://arxiv.org/abs/2310.15200).
...
Zero-shot results on LVIS minival at 640 pixels, from the [Ultralytics YOLO26 paper](https://arxiv.org/abs/2606.03748).

!!! tip "LVIS minival accuracy"

    === "Text / visual prompt"

        Each cell reads **text prompt / visual prompt**. Parameters and FLOPs are for the detection configuration the paper evaluates.
...
- **Against the earlier YOLOE families.** YOLOE-26 inherits [YOLO26](yolo26.md)'s NMS-free end-to-end head and covers five scales (n/s/m/l/x) against the earlier three (s/m/l), and leads at every matching scale in [Performance](#performance).
...
All three models below take a text prompt, but only YOLOE and SAM 3 return masks, and they answer different questions:
...
- **Zero-shot accuracy is well below a model trained on your classes.** The prompted checkpoints land roughly in the 25-40 mAP band on LVIS minival; a closed-set YOLO trained on your own data will beat that on those classes. Reach for YOLOE to cover classes you cannot train for, not to replace training.
- **Rare categories are the weak spot.** The mAP<sub>r</sub> column reports accuracy on LVIS's rare classes specifically, and it sits below the common and frequent columns across the range. Check it rather than the headline mAP when your targets are unusual.
...
- **Large prompt sets cost latency.** The prompt embeddings are computed once, but they are compared against region features on every forward pass. Measured on `yoloe-26s-seg` on CPU, a forward pass grows about 19% going from 80 to 1,203 classes and about 89% at the full 4,585-name vocabulary. Reported FLOPs do not move at all, because the region-text similarity is not counted, so the profile will not warn you.
...
- **Hardware.** Inference wants an NVIDIA GPU with 4-8 GB of VRAM; the `n` and `s` scales run on edge GPUs such as [Jetson](../guides/nvidia-jetson.md) or on CPU at reduced resolution. Fine-tuning needs a single GPU. [Batch inference](../modes/predict.md) works directly, and visual prompts can differ per image in the same call.
...
!!! warning
    ...
    The from-scratch trainers inherit `YOLOETrainer`, which refuses `compile=True`. Pass `compile=False` (the default) here. The fine-tuning trainers above do not carry that restriction.
...
The visual-prompt and prompt-free checkpoints start from that trained text-prompt model and update one module each, with everything else frozen: `YOLOESegVPTrainer` trains only SAVPE, and `YOLOEPEFreeTrainer` trains only the specialized prompt embedding. The [upstream YOLOE repository](https://github.com/THU-MIG/yoloe) carries the full recipes for the v8-scale models.
...
The detection head carries a mask prediction branch, as in YOLOv8-Seg, but it works for any prompted class rather than a fixed list. Masks come back with every prediction and are read from `results[0].masks`, so no second model is needed for pixel-precise boundaries.
