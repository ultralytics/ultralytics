...
class TensorRTBackend(BaseBackend):
    ...
    def load_model(self, weight: str | Path) -> None:
        ...
        self.model = engine
        self.input_name = next(n for n in self.bindings if n not in self.output_names)
        # For static engines without Ultralytics metadata, derive imgsz and use stride=1
        # so check_imgsz doesn't round the fixed input resolution (e.g. 700 → 704).
        if not self.dynamic and not metadata:
