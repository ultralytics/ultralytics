...
3. **Delete superseded code and modify existing files before creating new ones**: Bugfixes are net-negative by default; a net-positive bugfix needs a one-sentence PR justification explaining why deletion or relocation was impossible. A new file must first prove it cannot fit cleanly in an existing owner.
