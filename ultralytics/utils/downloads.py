...
def check_disk_space(
    ...
    # Insufficient space
    text = (
        f"Insufficient free disk space {free / (1 << 20):.1f} MB < {int(file_bytes * sf) / (1 << 20):.1f} MB required, "
        f"Please free {int(file_bytes * sf - free) / (1 << 20):.1f} MB additional disk space and try again."
    )
    if hard:
