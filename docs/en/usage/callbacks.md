...
| `on_fit_epoch_end`          | Triggered at the end of each fit epoch (train + val), **after** validation and any checkpoint save. Validation metrics and fitness are available. Also called during final best-model evaluation, where no checkpoint save occurs. |
