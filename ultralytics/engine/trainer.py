...
class BaseTrainer:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks: dict | None = None):
        ...
        # DDP ranks pin the device already set by ultralytics/utils/dist.py before the process group was initialized
        self.device = (
            select_device(self.args.device)
            if RANK == -1
            else torch.device("cuda", int(self.args.device.split(",")[LOCAL_RANK]))
        )
        self.validator = None
        self.metrics = None
        self.plots = {}
