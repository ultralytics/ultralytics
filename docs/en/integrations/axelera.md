...
| `fraction` | `float`, `int`, or `list` | `1.0`       | Calibration subset as a ratio, image count, or `[train, val]` ratios/counts; a list uses the entry selected by `split` (100-400 images recommended).                                                                 |
