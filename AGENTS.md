...
2. Batch reviews before the final gate: read the live PR-head SHA and pull until local `HEAD` matches, then record it and inside the implementation session launch focused reviewer children sharing context to review the full base-to-recorded-head PR diff for correctness, performance and production readiness, and Core Principles/minimalism; every reviewer must continue after the first blocker and report all findings. Fix and commit the batch, then have the same children review `<recorded-sha>..HEAD` plus affected invariants, advancing the recorded SHA after each clean delta. Push, wait for automation, pull, and repeat delta review until local `HEAD` equals the live PR head. Then run one fresh `codex exec` cold-context review of the exact full PR diff at the current head, record its final-reviewed SHA, and require it to continue after blockers, report all findings, and say LGTM only if none remain. Before handoff or merge, require the live PR head to equal the final-reviewed SHA; otherwise pull and restart one full review.
