...
class AxeleraBackend(BaseBackend):
    ...
    def forward(self, im: torch.Tensor) -> np.ndarray | list[np.ndarray]:
        ...
        A compiled model accepts a single image, so batches go through the Axelera scheduler via `batch()`, which
        takes the whole tensor as one argument (its varargs are one entry per model input, not per image) and
        returns one result per image in input order, each keeping its singleton batch dimension.
