...
class ConfusionMatrix(DataExportMixin):
    ...
    def process_batch(
        ...
        no_pred = detections["cls"].size(0) == 0
    ...
    def plot_matches(self, img: torch.Tensor, im_file: str, save_dir: Path) -> None:
        ...
        for i, mtype in enumerate(["GT", "FP", "TP", "FN"]):
            ...
            if "conf" not in mbatch:
                mbatch["conf"] = torch.tensor([1.0] * mbatch["bboxes"].size(0), device=img.device)
            mbatch["batch_idx"] = torch.ones(mbatch["bboxes"].size(0), device=img.device) * i
            for k in mbatch.keys():
                labels[k] += mbatch[k]

        labels = {k: torch.stack(v, 0) if len(v) else v for k, v in labels.items()}
        if self.task != "obb" and labels["bboxes"].size(0):
