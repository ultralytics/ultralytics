...
def _get_explorer():
    ...
    thread = Thread(target=exp.create_embeddings_table, kwargs={"force": st.session_state.get("force_recreate_embeddings")})
    thread.start()
...
def similarity_form(selected_imgs):
    ...
    with st.form("similarity_form"):
        ...
        with subcol1:
            limit = st.number_input("limit", min_value=None, max_value=None, value=25, label_visibility="collapsed", key="limit")
...
def utralytics_explorer_docs_callback():
    with st.container(border=True):
        st.image("https://raw.githubusercontent.com/ultralytics/assets/main/logo/Ultralytics_Logotype_Original.svg", width=100)
        st.markdown("<p>This demo is built using Ultralytics Explorer API. Visit <a href=''>API docs</a> to try examples & learn more</p>", unsafe_allow_html=True, help=None)
    ...
    with col1:
        ...
        with subcol4:
            start_idx = st.number_input(
                "Start Index", min_value=0, max_value=total_imgs, value=0, key="start_index", label_visibility="collapsed")
        with subcol5:
