...
It limits detections returned by prediction and validation. For end-to-end and embedded-NMS exports, the limit is part of the graph, so re-export to change it; CoreML embedded NMS is the exception and has no detection cap. End-to-end output can contain fewer candidates when the image supplies fewer than `max_det` anchors.
