    ...
    def _setup_ddp(self):
        ...
        if self.device.type == "xpu":
            try:
                ...
                raise ImportError(
                    "CCL backend for XPU is not available, install 'intel-extension-for-pytorch' and 'oneccl_bind_pt'. "
                    "See https://pytorch-extension.intel.com/installation?platform=gpu for install instructions."
                ) from e

            # ZE_AFFINITY_MASK from select_device() remaps the requested XPUs to 0..world_size-1
