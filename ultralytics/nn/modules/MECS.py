...
class MECS(nn.Module):
    def __init__(self, in_channels, out_channels, channel_attention_reduce=4):
        ...
        self.channel_attention = ChannelAttention(
            input_channels=in_channels, internal_neurons=in_channels // channel_attention_reduce
        )
    ...
    def forward(self, inputs):
        # 全局感知机
        inputs = self.pointwise_conv(
            inputs
        )  # 图中我画是先进入channel_attention，但是代码实际先进入pointwise_conv，不影响哈。
