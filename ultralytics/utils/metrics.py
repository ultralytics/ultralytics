...
class HumanMetrics(DetMetrics):
    """Calculate and aggregate detection metrics plus human attribute accuracies.

    Attributes:
        attrs_stats (dict[str, list]): Per-batch accuracy tensors for each human attribute.
        attrs_accuracy (dict[str, float]): Mean accuracy for each human attribute.

    Methods:
        process: Process detection stats and the accumulated human attribute accuracies.
        keys: Return a list of keys for accessing specific metrics.
        mean_results: Return the mean detection results extended with the attribute accuracies.
        fitness: Return the fitness weighted across mAP50-95 and the five attribute accuracies.
    """

    def __init__(self, names: dict[int, str] | None = None) -> None:
        """Initialize the HumanMetrics class with class names.

        Args:
            names (dict[int, str], optional): Dictionary of class names.
        """
        super().__init__(names)
        self.attrs_stats = {"weight": [], "height": [], "gender": [], "age": [], "ethnicity": []}
        self.attrs_accuracy = dict.fromkeys(self.attrs_stats, 0.0)

    def process(self, save_dir: Path = Path("."), plot: bool = False, on_plot=None) -> dict[str, np.ndarray]:
        """Process detection statistics and reduce the accumulated human attribute accuracies.
    ...
    def mean_results(self) -> list[float]:
        """Return the mean detection results extended with the human attribute accuracies."""
        return DetMetrics.mean_results(self) + list(self.attrs_accuracy.values())

    @property
    def fitness(self) -> float:
        """Return fitness as a weighted combination of mAP50-95 and the human attribute accuracies."""
        # weights for [P, R, mAP@0.5, mAP@0.5:0.95, acc_w, acc_h, acc_g, acc_a, acc_e]
        w = [0.0, 0.0, 0.0, 0.2, 0.16, 0.16, 0.16, 0.16, 0.16]
        return float((np.array(self.mean_results()) * w).sum())
