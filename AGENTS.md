...
2. Batch reviews before the final gate: read the live PR-head SHA and pull until local `HEAD` matches, then record it and inside the implementation session launch focused reviewer children sharing context to review the full base-to-recorded-head PR diff for correctness, performance and production readiness, and Core Principles/minimalism; every reviewer must continue after the first blocker and report all findings. Fix and commit the batch, then have the same children review `<recorded-sha>..HEAD` plus affected invariants, advancing the recorded SHA after each clean delta. Once clean, run one fresh `codex exec` cold-context review of the exact full PR diff at the current head; it must continue after blockers, report all findings, and say LGTM only if none remain. Before handoff or merge, reread the live PR-head SHA; if it changed, pull and restart one full review.
