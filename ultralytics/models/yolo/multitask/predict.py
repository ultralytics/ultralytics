...
class MultiTaskPredictor(DetectionPredictor):
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        for i, (pred_seg, pred_kpt) in enumerate(zip(p_seg, p_pose)):
            ...
            pred_kpts = pred_kpt[:, 6:].view(len(pred_kpt_copy), *self.model.kpt_shape) if len(pred_kpt_copy) else pred_kpt_copy[:, 6:]
            ...
            if not len(pred_seg):  # save empty boxes
                ...
                masks = ops.process_mask(proto[i], pred_seg[:, 6:], pred_seg[:, :4], img.shape[2:], upsample=True)  # HWC
                pred_seg[:, :4] = ops.scale_boxes(img.shape[2:], pred_seg[:, :4], orig_img.shape)
            results.append(Results(orig_img, path=img_path, names=self.model.names, boxes=pred_seg[:, :6], masks=masks, keypoints=pred_kpts))
        return results
