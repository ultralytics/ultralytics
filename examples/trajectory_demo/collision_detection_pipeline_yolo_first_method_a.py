...
# 导入多锚点碰撞检测模块
from anchor_points import BicycleAnchors, MotorcycleAnchors, PedestrianAnchors, VehicleAnchors
...
class YOLOFirstPipelineA:
    def __init__(
        self, video_path, homography_path=None, output_base=None, skip_frames=3, model="yolo11n", min_track_length=3
    ):
        ...
        for d in [self.detection_dir, self.trajectory_dir, self.keyframe_dir, self.homography_dir, self.analysis_dir]:
            d.mkdir(parents=True, exist_ok=True)

        print(f"\n{'=' * 70}")
        print("YOLO-First 碰撞检测Pipeline (Method A - 导师推荐)")
        print(f"{'=' * 70}")
        print(f"时间戳: {timestamp}")
        print(f"结果目录: {self.run_dir}")
        print("执行顺序: YOLO → 轨迹(px) → 关键帧 → Homography(关键帧) → TTC")
    ...
    def save_keyframe_with_distance(self, video_path, frame_num, output_path, proximity_event):
        ...
        cv2.putText(
            frame, f"ID:{track_id_1}", (pt1[0] + 10, pt1[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2
        )

        cv2.circle(frame, pt2, 5, (0, 0, 255), -1)  # 红色圆点
        cv2.putText(
            frame, f"ID:{track_id_2}", (pt2[0] + 10, pt2[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2
        )
        ...
        if "multi_anchor_detailed" in proximity_event:
            try:
                ...
                if point1_px and point2_px:
                    ...
                    cv2.putText(
                        frame, dist_text, (mid_x - 80, mid_y - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 255), 2
                    )
                    ...
                    if ttc is not None:
                        ...
                        cv2.putText(
                            frame, ttc_text, (mid_x - 80, mid_y + 50), cv2.FONT_HERSHEY_SIMPLEX, 0.6, risk_color, 2
                        )
                    ...
                    cv2.putText(
                        frame,
                        obj1_part,
                        (anchor_pt1[0] - 50, anchor_pt1[1] - 20),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.5,
                        (0, 255, 0),
                        1,
                    ...
                    cv2.putText(
                        frame,
                        obj2_part,
                        (anchor_pt2[0] + 20, anchor_pt2[1] - 20),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.5,
                        (0, 0, 255),
                        1,
        ...
        cv2.putText(frame, distance_text, (mid_x - 130, mid_y + 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)

        # 显示物体类别信息
        class_text = f"{class_1} vs {class_2}"
        cv2.putText(frame, class_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
    ...
    def run_yolo_detection(self, conf_threshold=0.45):
        ...
        if self.skip_frames > 1:
            print(
                f"  处理中: 将处理 ~{expected_processing_frames} 帧 (从总共 {total_frames}帧中，每隔{self.skip_frames}帧处理一帧)..."
            )
        else:
                        ...
                        if distance < same_class_distance_threshold:
                            ...
                            if obj1["conf"] >= obj2["conf"]:
                                ...
                                class_name = {
                                    ...
                                    6: "train",
                                    7: "truck",
                                }.get(obj1["class"], f"class_{obj1['class']}")
                                print(
                                    f"  🔀 合并: Frame {frame:03d} - 两个 {class_name} (ID {obj1['track_id']}, ID {obj2['track_id']}) 距离 {distance:.1f}px < {same_class_distance_threshold}px"
                                )
                            else:
                                to_remove.add(i)
                                merged_count += 1
                                class_name = {
                                    ...
                                    6: "train",
                                    7: "truck",
                                }.get(obj2["class"], f"class_{obj2['class']}")
                                print(
                                    f"  🔀 合并: Frame {frame:03d} - 两个 {class_name} (ID {obj2['track_id']}, ID {obj1['track_id']}) 距离 {distance:.1f}px < {same_class_distance_threshold}px"
                                )
    ...
    def build_trajectories(self, all_detections):
        ...
        print("    坐标系统: 像素 + 世界坐标 (已在Step 2转换)")
        print("    速度单位: px/s (像素) + m/s (世界)")
    ...
    def detect_discontinuous_tracks(self, all_detections, max_gap_frames=None):
        ...
        """
        print("\n【Step 2.4: 轨迹间断检测】")
        print("  ℹ️  检测轨迹间断（出现→消失→重新出现，考虑frame skipping）")
        ...
        if suspicious_tracks:
            ...
            for track_id, info in sorted(suspicious_tracks.items()):
                print(
                    f"     - Track ID {track_id}: {info['total_frames']}帧 ({info['first_frame']}-{info['last_frame']})"
                )
                for gap_start, gap_end, gap_size in info["gaps"]:
                    print(
                        f"       └─ 间断: Frame {gap_start} → Frame {gap_end} (间隔 {gap_size} 帧，超过允许的 {self.skip_frames + 1} 帧)"
                    )
        else:
    ...
    def filter_short_tracks(self, all_detections, min_track_length=3):
        ...
        for frame_data in all_detections:
            ...
            new_frame_data["objects"] = [obj for obj in frame_data["objects"] if obj["track_id"] in valid_tracks]
            ...
            for track_b in track_ids[i + 1 :]:
                ...
                if 1 <= frame_gap <= max_gap_frames:  # 中间有间隔但不超过max_gap
                    ...
                    if distance < max_distance_pixels and class_a == first_b_class:
                        ...
                        merged_count += 1
                        print(
                            f"  🔗 合并: Track ID {track_b} (首次Frame {first_b_frame}) → ID {track_a} (末次Frame {frame_a})"
                        )
    ...
    def extract_key_frames(self, all_detections, tracks, world_distance_threshold=2.0, debug_threshold=5.0):
        ...
        print("  ℹ️  使用Step 2中Homography变换的世界坐标进行距离计算和空间验证")
        ...
        for frame_data in all_detections:
            ...
            for i in range(len(objects)):
                for j in range(i + 1, len(objects)):
                    ...
                    # 检查两个物体是否都在有效范围内
                    obj1_valid = (
                        world_x_min - world_margin <= x1_world <= world_x_max + world_margin
                        and world_y_min - world_margin <= y1_world <= world_y_max + world_margin
                    )
                    obj2_valid = (
                        world_x_min - world_margin <= x2_world <= world_x_max + world_margin
                        and world_y_min - world_margin <= y2_world <= world_y_max + world_margin
                    )
                    ...
                    if distance_meters < world_distance_threshold:
                        event = {
                            ...
                            "positions": {"obj1": {"x": x1_px, "y": y1_px}, "obj2": {"x": x2_px, "y": y2_px}},
                            "positions_world": {
                                "obj1": {"x": x1_world, "y": y1_world},
                                "obj2": {"x": x2_world, "y": y2_world},
                            },
                            # 多锚点碰撞分析信息
                            "multi_anchor": {"closest_parts": closest_parts, "risk_level": risk_level, "ttc": ttc},
                        }
        ...
        if excluded_count > 0:
            ...
            for pair in all_proximity_pairs:
                if pair["distance_meters"] >= world_distance_threshold:
                    print(
                        f"     - Frame {pair['frame']:03d}: Track {pair['track_ids'][0]}({pair['class_1']}) + Track {pair['track_ids'][1]}({pair['class_2']}) = {pair['distance_meters']:.2f}m"
                    )
    ...
    def filter_same_class_false_positives(self, proximity_events, same_class_distance_threshold=0.3):
        ...
        for event in proximity_events:
            ...
            if pair_key not in track_pair_analysis:
                track_pair_analysis[pair_key] = {"events": [], "frames": [], "distances": [], "classes": None}
        ...
        print("    条件1: 距离 < 0.1m")
        print("    条件4: 都是汽车类型 (car/truck/bus/motorcycle等) + 距离 < 0.5m")
        print("    条件2: 不合理类别组合 (person/motorcycle等) + 距离稳定 (std < 0.5m)")
        print("    条件3: Track ID对断断续续出现 + 平均距离 < 2.0m")
        ...
        for event in proximity_events:
            ...
            try:
                ...
                if obj1 is None or obj2 is None or track1_point is None or track2_point is None:
                    reason = []
                    if obj1 is None:
                        reason.append("obj1 not found")
                    if obj2 is None:
                        reason.append("obj2 not found")
                    if track1_point is None:
                        reason.append("track1_point not found")
                    if track2_point is None:
                        reason.append("track2_point not found")
    ...
    def transform_key_frames_to_world(self, proximity_events):
        ...
        """
        print("\n【Step 4: Homography 信息保存】")
        print("  ℹ️  注意: 坐标变换已在Step 2中完成（使用Homography）")

        if self.H is None:
            print("  ⚠️  未加载Homography")
            return proximity_events
        ...
        with open(report_path, "w") as f:
            ...
            if ttc_classified["rear_end_serious"]:
                ...
                for event in ttc_classified["rear_end_serious"][:5]:
                    ...
                    f.write(
                        f"  Frame {event['frame']}: TTC={ttc_str}, 距离={event['multi_anchor_detailed'].get('min_distance_meters', 0):.3f}m\n"
                    )
            ...
            if ttc_classified["rear_end_general"]:
                f.write(
                    f"【Rear-end - General Conflict (TTC 2.8-4.7s)】: {len(ttc_classified['rear_end_general'])} 个\n"
                )
            ...
            if ttc_classified["sideswipe_serious"]:
                f.write(
                    f"【Sideswipe - Serious Conflict (TTC 0-2.3s)】: {len(ttc_classified['sideswipe_serious'])} 个\n"
                )
            ...
            if ttc_classified["sideswipe_general"]:
                f.write(
                    f"【Sideswipe - General Conflict (TTC 2.3-4.2s)】: {len(ttc_classified['sideswipe_general'])} 个\n"
                )
            ...
            if not any(
                [
                    ttc_classified["rear_end_serious"],
                    ttc_classified["rear_end_general"],
                    ttc_classified["sideswipe_serious"],
                    ttc_classified["sideswipe_general"],
                ]
            ):
                ...
                for event in sorted_events[:10]:
                    ...
                    if "multi_anchor_detailed" in event:
                        ...
                        if ttc is not None and ttc > 0:
                            ...
                            # 根据approaching标志判断原因
                            if approaching:
                                f.write("TTC (时间碰撞): 无法计算 / Insufficient Speed\n")
                            else:
                                f.write("TTC (时间碰撞): 远离 / Separating\n")
    ...
    def run(self, conf_threshold=0.45):
        ...
        try:
            ...
            self.detect_discontinuous_tracks(all_detections, max_gap_frames=3)
            ...
            if not proximity_events:
                ...
                filtered_events = self.filter_same_class_false_positives(
                    proximity_events, same_class_distance_threshold=0.3
                )
            ...
            print(f"\n{'=' * 70}")
            print("✓ YOLO-First Pipeline (Method A) 完成！")
            print(f"{'=' * 70}")
            ...
            print("\n文件夹结构:")
            print("  1_yolo_detection/")
            print("    ├── detections_pixel.json")
            print("    ├── detection_stats.json")
            print("    └── *.jpg (所有有检测的帧)")
            print("  2_trajectories/")
            print("    ├── tracks.json")
            print("    └── track_stats.json")
            print("  3_key_frames/")
            print("    ├── proximity_events.json")
            print("    └── *.jpg (接近事件的关键帧)")
            print("  4_homography_transform/")
            print("    ├── homography.json")
            print("    └── transformed_key_frames.json")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--homography", type=str, default=None, help="Homography JSON路径 (可选)")
    parser.add_argument("--output", type=str, default="../../results", help="结果基础目录")
    parser.add_argument("--conf", type=float, default=0.45, help="YOLO置信度阈值 (越高=越严格，减少误检) (默认: 0.45)")
    parser.add_argument(
        "--skip-frames",
        type=int,
        default=3,
    ...
    parser.add_argument(
        "--min-track-length", type=int, default=3, help="最小轨迹长度(帧数)，短于此的轨迹被认为是误检并排除 (默认: 3)"
    )
