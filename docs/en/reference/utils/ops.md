...
!!! abstract "Summary"
    ...
    === "<span class="doc-kind doc-kind-function">Functions</span>"
        ...
        - [`linear_sum_assignment`](#ultralytics.utils.ops.linear_sum_assignment)
        - [`_linear_sum_assignment_numpy`](#ultralytics.utils.ops._linear_sum_assignment_numpy)


## Class `ultralytics.utils.ops.Profile` {#ultralytics.utils.ops.Profile}

```python
Profile(t: float = 0.0, device: torch.device | None = None)
```
...
Use as a decorator with @Profile() or as a context manager with 'with Profile():'. Provides accurate timing measurements with accelerator synchronization support.
...
| `device` | `torch.device, optional` | Device used for model inference to enable accelerator synchronization. | `None` |
...
| [`time`](#ultralytics.utils.ops.Profile.time) | Get current time with accelerator synchronization if applicable. |
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L20-L74">View on GitHub</a>
```python
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L56-L59">View on GitHub</a>
```python
def __enter__(self):
    """Start timing."""
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L61-L64">View on GitHub</a>
```python
def __exit__(self, type, value, traceback):
    """Stop timing."""
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L66-L68">View on GitHub</a>
```python
def __str__(self):
    """Return a human-readable string representing the accumulated elapsed time."""
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L70-L74">View on GitHub</a>
```python
def time(self):
    """Get current time with accelerator synchronization if applicable."""
...
Converts a single segment label to a box label by finding the minimum and maximum x and y coordinates of the polygon clipped to the image, so segments crossing the image boundary keep their visible extent. Segments already inside the image return immediately without clipping.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L77-L119">View on GitHub</a>
```python
def segment2box(segment: np.ndarray, width: int = 640, height: int = 640) -> np.ndarray:
    """Convert segment coordinates to bounding box coordinates.
...
| `boxes` | `torch.Tensor \| np.ndarray` | Bounding boxes to rescale in format (N, 4). | *required* |
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L122-L161">View on GitHub</a>
```python
def scale_boxes(
    img1_shape: tuple[int, int],
    boxes: torch.Tensor | np.ndarray,
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L164-L176">View on GitHub</a>
```python
def make_divisible(x: int, divisor):
    """Return the smallest number >= x that is divisible by the given divisor.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L179-L204">View on GitHub</a>
```python
def clip_boxes(boxes, shape):
    """Clip bounding boxes to image boundaries.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L207-L228">View on GitHub</a>
```python
def clip_coords(coords, shape):
    """Clip line coordinates to image boundaries.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L231-L248">View on GitHub</a>
```python
def xyxy2xywh(x):
    """Convert bounding box coordinates from (x1, y1, x2, y2) format to (x, y, width, height) format where (x1, y1) is
    the top-left corner and (x2, y2) is the bottom-right corner.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L251-L267">View on GitHub</a>
```python
def xywh2xyxy(x):
    """Convert bounding box coordinates from (x, y, width, height) format to (x1, y1, x2, y2) format where (x1, y1) is
    the top-left corner and (x2, y2) is the bottom-right corner. Note: ops per 2 channels faster than per channel.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L270-L291">View on GitHub</a>
```python
def xywhn2xyxy(x, w: int = 640, h: int = 640, padw: int = 0, padh: int = 0):
    """Convert normalized bounding box coordinates to pixel coordinates.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L294-L317">View on GitHub</a>
```python
def xyxy2xywhn(x, w: int = 640, h: int = 640, clip: bool = False, eps: float = 0.0):
    """Convert bounding box coordinates from (x1, y1, x2, y2) format to normalized (x, y, width, height) format. x, y,
    width and height are normalized to image dimensions.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L320-L332">View on GitHub</a>
```python
def xywh2ltwh(x):
    """Convert bounding box format from [x, y, w, h] to [x1, y1, w, h] where x1, y1 are top-left coordinates.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L335-L347">View on GitHub</a>
```python
def xyxy2ltwh(x):
    """Convert bounding boxes from [x1, y1, x2, y2] to [x1, y1, w, h] format.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L350-L362">View on GitHub</a>
```python
def ltwh2xywh(x):
    """Convert bounding boxes from [x1, y1, w, h] to [x, y, w, h] where xy1=top-left, xy=center.
...
| Name | Type | Description | Default |
| --- | --- | --- | --- |
| `x` | `np.ndarray \| torch.Tensor` | Input box corners with shape (N, 8) or (N, 4, 2) in [xy1, xy2, xy3, xy4] format. Polygons with more than four points are accepted in the same two layouts, (N, 2P) or (N, P, 2), and are reduced to their minimum-area rectangle. | *required* |

**Returns**

| Type | Description |
| --- | --- |
| `np.ndarray \| torch.Tensor` | Converted data in [cx, cy, w, h, rotation] format with shape (N, 5). The parameterization is canonical rather than the caller's: w is the longer side and rotation is in radians from [-pi/4, 3pi/4), so a box given with w < h comes back with w and h swapped and its angle shifted by pi/2 modulo pi. |

<details>
<summary>Source code in <code>ultralytics/utils/ops.py</code></summary>

<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L365-L397">View on GitHub</a>
```python
def xyxyxyxy2xywhr(x):
    """Convert batched Oriented Bounding Boxes (OBB) from [xy1, xy2, xy3, xy4] to [xywh, rotation] format.
...
| Name | Type | Description | Default |
| --- | --- | --- | --- |
| `x` | `np.ndarray \| torch.Tensor` | Boxes in [cx, cy, w, h, rotation] format with shape (N, 5) or (B, N, 5). Rotation is in radians and is neither range-checked nor normalized; the box is not canonicalized, so converting the (N, 4, 2) corners back with xyxyxyxy2xywhr returns the canonical form of the same rectangle rather than these values. | *required* |
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L400-L429">View on GitHub</a>
```python
def xywhr2xyxyxyxy(x):
    """Convert batched Oriented Bounding Boxes (OBB) from [xywh, rotation] to [xy1, xy2, xy3, xy4] format.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L432-L444">View on GitHub</a>
```python
def ltwh2xyxy(x):
    """Convert bounding box from [x1, y1, w, h] to [x1, y1, x2, y2] where xy1=top-left, xy2=bottom-right.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L447-L460">View on GitHub</a>
```python
def segments2boxes(segments):
    """Convert segment coordinates to bounding box labels in xywh format.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L463-L483">View on GitHub</a>
```python
def resample_segments(segments, n: int = 1000):
    """Resample segments to n points each using linear interpolation.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L486-L506">View on GitHub</a>
```python
def crop_mask(masks: torch.Tensor, boxes: torch.Tensor) -> torch.Tensor:
    """Crop masks to bounding box regions.
...
| Type | Description |
| --- | --- |
| `torch.Tensor` | A binary mask tensor of shape [n, h, w], where n is the number of masks after NMS. When upsample=True h and w match the input image size; otherwise they are the prototype mask resolution. |

<details>
<summary>Source code in <code>ultralytics/utils/ops.py</code></summary>

<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L509-L537">View on GitHub</a>
```python
def process_mask(protos, masks_in, bboxes, shape, upsample: bool = False):
    """Apply masks to bounding boxes using mask head output.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L540-L565">View on GitHub</a>
```python
def process_mask_native(protos, masks_in, bboxes, shape):
    """Apply masks to bounding boxes using mask head output with native upsampling.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L568-L605">View on GitHub</a>
```python
def scale_masks(
    masks: torch.Tensor,
    shape: tuple[int, int],
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L608-L640">View on GitHub</a>
```python
def scale_coords(img1_shape, coords, img0_shape, ratio_pad=None, normalize: bool = False, padding: bool = True):
    """Rescale segment coordinates from img1_shape to img0_shape.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L643-L658">View on GitHub</a>
```python
def regularize_rboxes(rboxes):
    """Regularize rotated bounding boxes to range [0, pi/2).
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L661-L689">View on GitHub</a>
```python
def masks2segments(masks: np.ndarray | torch.Tensor, strategy: str = "all") -> list[np.ndarray]:
    """Convert masks to segments using contour detection.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L692-L701">View on GitHub</a>
```python
def convert_torch2numpy_batch(batch: torch.Tensor) -> np.ndarray:
    """Convert a batch of FP32 torch tensors to NumPy uint8 arrays, changing from BCHW to BHWC layout.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L704-L713">View on GitHub</a>
```python
def clean_str(s):
    """Clean a string by replacing special characters with '_' character.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L716-L718">View on GitHub</a>
```python
def empty_like(x):
    """Create empty torch.Tensor or np.ndarray with same shape and dtype as input."""
...
## Function `ultralytics.utils.ops.linear_sum_assignment` {#ultralytics.utils.ops.linear\_sum\_assignment}
...
| Name | Type | Description | Default |
| --- | --- | --- | --- |
| `cost_matrix` | `np.ndarray \| torch.Tensor` | Cost matrix with shape (N, M); `+inf` forbids assignments. | *required* |
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L724-L768">View on GitHub</a>
```python
def linear_sum_assignment(cost_matrix):
    """Solve the rectangular linear sum assignment problem (minimum-cost one-to-one matching).
...
## Function `ultralytics.utils.ops._linear_sum_assignment_numpy` {#ultralytics.utils.ops.\_linear\_sum\_assignment\_numpy}
...
| Name | Type | Description | Default |
| --- | --- | --- | --- |
| `a` | `np.ndarray` | Float64 cost matrix of shape (N, M); `+inf` forbids assignments. | *required* |
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/utils/ops.py#L771-L816">View on GitHub</a>
```python
def _linear_sum_assignment_numpy(a):
    """Solve the rectangular linear sum assignment problem with NumPy (Jonker-Volgenant SciPy-free fallback).
