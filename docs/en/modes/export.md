...
Both YOLO27 detection designs are intended to deploy through the same export interface; the [YOLO27 preview](../models/yolo27.md#supported-tasks-and-modes) lists planned export support per task. YOLO27 is coming soon and not yet available, so the examples on this page export released YOLO26 models.
