...
class Yolov8Inference:
    ...
    def draw_result(self, img: np.ndarray, result: Dict) -> np.ndarray:
        ...
        for box, idx, score in zip(boxes, idxes, scores):
            ...
            cv2.rectangle(img, (x1, y1), (x1 + label_size[0], y1 + label_size[1] + baseline), color, -1)
            cv2.putText(img, label, (x1, y1 + label_size[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
        return img


def parse_args():
    parser = argparse.ArgumentParser('yolov8 onnx inference demo')
    parser.add_argument('demo', type=str, help='demo type, image or stream')
    parser.add_argument('model', type=str, help='onnx model path')
    parser.add_argument('input', type=str, default='0', help='camera id | video path | image path')
    parser.add_argument('--score_thr', type=float, default=0.25, help='score threshold')
    parser.add_argument('--nms_thr', type=float, default=0.5, help='nms threshold')
    parser.add_argument('--input_size', type=int, default=640, help='inpur size')
    parser.add_argument('--classes_txt', type=str, default='', help='class names txt file.')
...
def demo_image(args):
    ...
    yolo = Yolov8Inference(args.model, input_size, class_names, args.score_thr, args.nms_thr)
