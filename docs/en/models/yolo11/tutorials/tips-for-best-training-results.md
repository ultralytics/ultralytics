---
comments: true
description: Practical tips to get the best detection accuracy when training Ultralytics YOLO11. Data quality, augmentation, batch size, learning rate, and more.
keywords: YOLO11, training tips, best results, data quality, augmentation, learning rate, batch size, pretrained weights, Ultralytics
canonical: https://docs.ultralytics.com/models/yolo11/tutorials/tips-for-best-training-results/
---
...
<script type="application/ld+json">
{
  ...
  "@type": "TechArticle",
  "headline": "Tips for Best YOLO11 Training Results",
  "description": "Practical tips to get the best detection accuracy when training Ultralytics YOLO11. Data quality, augmentation, batch size, learning rate, and more.",
  "url": "https://docs.ultralytics.com/models/yolo11/tutorials/tips-for-best-training-results/",
  ...
  "mainEntityOfPage": "https://docs.ultralytics.com/models/yolo11/tutorials/tips-for-best-training-results/"
}
</script>

# Tips for Best YOLO11 Training Results

<!-- NOTE FOR MURAT: Please verify all tips apply correctly to YOLO11's architecture. Add any YOLO11-specific flags or defaults that differ from previous YOLO versions. Add representative before/after mAP numbers showing the impact of key tips (e.g. pretrained vs scratch, with/without mosaic). Confirm the model size table values (mAP, speed) are up to date. -->

Getting the best accuracy from [Ultralytics YOLO11](../../../models/yolo11.md) on your custom dataset depends on more than just training duration. This guide consolidates practical advice on data quality, model configuration, augmentation, and training hygiene.
...
Always start from a pretrained YOLO11 checkpoint rather than training from scratch. Pretrained weights encode rich visual features from large-scale datasets and dramatically reduce the data and compute needed to reach peak accuracy.

```bash
# Good — fine-tune from pretrained weights
yolo train model=yolo11n.pt data=my_dataset.yaml epochs=100

# Avoid — training from scratch requires far more data and epochs
yolo train model=yolo11n.yaml data=my_dataset.yaml epochs=300
```

| Starting point | Epochs to convergence | Typical mAP improvement |
|---|---|---|
| `yolo11n.pt` (pretrained) | 100–150 | Baseline |
| `yolo11n.yaml` (scratch) | 300+ | −5 to −15 mAP |

## 3. Match Image Size to Your Data

YOLO11 trains at `imgsz=640` by default. If your objects are small, consider training at a larger resolution.

```bash
# Default
yolo train model=yolo11n.pt data=my_dataset.yaml imgsz=640

# Larger resolution for small objects
yolo train model=yolo11n.pt data=my_dataset.yaml imgsz=1280
```
...
yolo train model=yolo11n.pt data=my_dataset.yaml batch=-1

# Or set explicitly
yolo train model=yolo11n.pt data=my_dataset.yaml batch=32
```
...
yolo train model=yolo11n.pt data=my_dataset.yaml mosaic=1.0 mixup=0.1

# Close mosaic in the final epochs to let the model stabilise
yolo train model=yolo11n.pt data=my_dataset.yaml mosaic=1.0 close_mosaic=10
```
...
yolo train model=yolo11n.pt data=my_dataset.yaml lr0=0.01

# Small dataset fine-tuning — lower LR reduces catastrophic forgetting
yolo train model=yolo11n.pt data=my_dataset.yaml lr0=0.001 lrf=0.1
```
...
```bash
yolo train model=yolo11n.pt data=my_dataset.yaml patience=50
```
...
When exploring a new dataset, start with the nano (`yolo11n`) model for fast iteration. Only scale to larger variants once you have a validated data pipeline and know the task difficulty.

| Model | mAP<sup>val</sup> 50-95 | Recommended use |
|---|---|---|
| `yolo11n.pt` | 39.5 | Rapid prototyping, edge deployment |
| `yolo11s.pt` | 47.0 | Balanced speed/accuracy |
| `yolo11m.pt` | 51.5 | Most custom datasets |
| `yolo11l.pt` | 53.4 | High-accuracy production |
| `yolo11x.pt` | 54.7 | Maximum accuracy (benchmark) |
