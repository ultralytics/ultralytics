...
class BaseExporter:
    ...
    def __init__(self, save_dir=None, args=None):
        self.logger = LOGGER
    ...
    def __call__(self,
        ...
        imgsz = [check_imgsz(x, gs) for x in imgsz]  # verify imgsz is gs-multiple
        ...
        if engine:  # TensorRT required before ONNX
            f[1], _ = self.export_engine(model, im, file, half, dynamic, simplify, workspace, verbose)
        if onnx or xml:  # OpenVINO requires ONNX
            f[2], _ = self.export_onnx(model, im, file, opset, dynamic, simplify)
        if xml:  # OpenVINO
            f[3], _ = self.export_openvino(file, metadata, half)
        if coreml:  # CoreML
        ...
        if any((saved_model, pb, tflite, edgetpu, tfjs)):  # TensorFlow formats
            ...
            if tflite or edgetpu:
                ...
                self.add_tflite_metadata(f[8] or f[7], metadata, num_outputs=len(s_model.outputs))
            if tfjs:
                f[9], _ = self.export_tfjs(file)
        if paddle:  # PaddlePaddle
            f[10], _ = self.export_paddle(model, im, file, metadata)
    ...
    @try_export
    def export_openvino(self, file, metadata, half, prefix=colorstr('OpenVINO:')):
    ...
    @try_export
    def export_paddle(self, model, im, file, metadata, prefix=colorstr('PaddlePaddle:')):
    ...
    @try_export
    def export_coreml(self, model, im, file, int8, half, prefix=colorstr('CoreML:')):
    ...
    @try_export
    def export_edgetpu(self, file, prefix=colorstr('Edge TPU:')):
    ...
    @try_export
    def export_tfjs(self, file, prefix=colorstr('TensorFlow.js:')):
