...
def draw_contact_points(frame, sample, obj_id, color=(0, 255, 0), alpha=0.7):
    ...
    for i, (x, y) in enumerate(points):
        ...
        # 标注点的名字
        label = point_names[i]
        cv2.putText(frame, label, (x + 15, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
...
def draw_collision_info(frame, near_miss_event, obj1_sample, obj2_sample, fps=30.0):
    ...
    cv2.rectangle(frame, (x_offset - 10, y_offset - 20), (w - 10, y_offset + box_height), (0, 0, 0), -1)

    # 文字
    for i, line in enumerate(info_lines):
        color = (0, 0, 255) if "CRITICAL" in line or "HIGH" in line else (0, 255, 255)
        cv2.putText(frame, line, (x_offset, y_offset + i * line_height), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
...
def visualize_collision_events(video_path, near_misses, tracks, output_dir, top_k=10):
    ...
    for event_idx, event in enumerate(sorted_events[:top_k]):
        ...
        output_filename = (
            f"collision_event_{event_idx + 1:03d}_frame_{frame_num:05d}_obj{obj1_id}_vs_obj{obj2_id}_{risk_level}.jpg"
        )
