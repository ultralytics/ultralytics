...
def run(source: str, weights: str, output: str, homography_path: str = None, conf_threshold: float = 0.5, 
        distance_threshold: float = 5.0, auto_subdir: bool = True):
    """
    ...
    if homography_path:
        ...
        if H is not None:
            print(f"✓ Homography矩阵已加载，距离单位为米")
        else:
            print(f"⚠ 无法加载Homography矩阵，距离单位为像素")
    ...
    for result in model.track(source=source, stream=True, persist=True, conf=conf_threshold):
        ...
        if frame_events:
            ...
            for det in detections:
                ...
                cv2.putText(frame_marked, f"ID{obj_id}", (int(x1), int(y1)-10),
            ...
            for event in frame_events:
                ...
                cv2.putText(frame_marked, text, (10, text_y),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
                text_y += 30
            
            cv2.imwrite(frame_path, frame_marked)
            print(f"帧 {frame_idx:4d} (t={frame_idx/fps:6.2f}s): 检测到 {len(frame_events)} 个事件 → {frame_filename}")
    ...
    print(f"  - event_frames/: 包含所有有事件的帧截图")
    print(f"  - near_misses.json: 详细的事件数据")
    print(f"  - event_frames.json: 事件帧索引")
    print(f"  - event_summary.txt: 事件汇总报告")
    print("=" * 70 + "\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--distance-threshold', type=float, default=5.0, 
                       help='触发事件截图的距离阈值（米或像素）')
    
    args = parser.parse_args()
    
    run(args.source, args.weights, args.output, 
