...
"""
Regression test for BYTETracker index collision in second-association path.
...
@pytest.fixture
def tracker():
    """Return a fresh BYTETracker with the thresholds from the PR deduction."""
    args = IterableSimpleNamespace(
        track_high_thresh=0.6,  # as specified in the deduction
        track_low_thresh=0.1,  # as specified in the deduction
        match_thresh=0.8,  # high enough that tiny bbox shifts still match
        new_track_thresh=0.6,  # activate tracks on first frame
        track_buffer=30,
        fuse_score=True,
    )
