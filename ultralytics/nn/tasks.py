...
class YOLOAnomalyV2Model(DetectionModel):
    ...
    def __init__(
        ...
        # Training-only prior-mask augmentation (data-space regularization, not architecture).
    ...
    def _install_backbone_taps(self, layer_indices: list[int]) -> None:
        ...
        def _make_hook(idx: int):
            def _hook(_module, _inp, out):
                # Detached capture on every forward for load_support_set / prior_mode="heatmap".
                # Detach keeps the graph out; cost is one small held activation per tapped layer
                # until the next forward clears the dict.
    ...
    def __getstate__(self):
        ...
        heatmap, aux/seg/mask staging) are transient — repopulated on the next forward — but if a
        forward ran just before ``torch.save`` they get pickled into the checkpoint and bloat it.
    ...
    def _polygon_union_prior(self, masks, batch_idx, batch_size):
        ...
        if masks.dim() == 3 and masks.shape[0] == batch_size:
            ...
            if self.training and self.mask_augmenter.mask_box_drop_p > 0.0 and masks.numel() > 0:
                keep = torch.rand(masks.shape[0], device=masks.device) > self.mask_augmenter.mask_box_drop_p
                masks = masks[keep]
        ...
        if self.training and self.mask_augmenter.mask_jitter > 0.0:
    ...
    def _fragment_mask(self, mask):
        ...
        for i in range(b):
            ...
            for _ in range(self.mask_augmenter.mask_fragment_n):
    ...
    def _drop_polygon_instances(self, masks):
        ...
        Polygon equivalent of ``mask_box_drop_p`` in ``_augment_prior_bboxes``.
        """
        ids = masks.unique()
        ids = ids[ids > 0]  # exclude background
        if len(ids) == 0:
            return masks
        keep = torch.rand(len(ids), device=masks.device) > self.mask_augmenter.mask_box_drop_p
        if keep.all():
            return masks
        drop_ids = ids[~keep]
        out = masks.clone()
    ...
    def _translate_prior(self, prior):
        ...
        Polygon equivalent of ``mask_jitter`` in ``_augment_prior_bboxes``.
        Uses bilinear sampling so sub-pixel shifts produce soft edges (like the gauss
        rendering the jittered bbox would).
        """
        B, _, H, W = prior.shape
        dev = prior.device
        off = (torch.rand(B, 2, device=dev) * 2 - 1) * (self.mask_augmenter.mask_jitter * self.mask_size)
