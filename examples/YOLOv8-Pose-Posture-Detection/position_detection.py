...
class PoseDetector:
    ...
    def detect_position(self, keypoints):
        ...
        valid_points = all([
            l_shoulder[2] > 0.5, r_shoulder[2] > 0.5,
            l_hip[2] > 0.5, r_hip[2] > 0.5,
        ])
        ...
        # Calculate centers
        center_shoulders = ((l_shoulder[0] + r_shoulder[0]) / 2, 
                        (l_shoulder[1] + r_shoulder[1]) / 2)
        
        center_hips = ((l_hip[0] + r_hip[0]) / 2, 
                       (l_hip[1] + r_hip[1]) / 2)
        ...
        if l_knee[2] > 0.5:
            left_knee_corner = self.calculate_angle(
                l_shoulder[:2], l_hip[:2], l_knee[:2]
            )
            
        if r_knee[2] > 0.5:
            right_knee_corner = self.calculate_angle(
                r_shoulder[:2], r_hip[:2], r_knee[:2]
            )
            ...
            if result.keypoints is not None:
                for person_keypoints in result.keypoints.data:
                    ...
                    if keypoints[0][2] > 0.5:  # If the nose is visible
                        ...
                        cv2.putText(frame, position, (x - 50, y - 20),
