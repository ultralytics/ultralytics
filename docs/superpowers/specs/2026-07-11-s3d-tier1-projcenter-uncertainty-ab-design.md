...
**Goal:** raise KITTI Car *Moderate* **AP3D@0.7** (baseline 4.2; @0.5 is 34.3) by fixing the two structural localization gaps the code trace and the recent-literature survey both flagged as top levers.
...
Uncertainty is *trained once* (NLL); how it is *used* is a decode-time toggle, so fusion and score-weighting are isolated on the same checkpoint.
...
- **Decode knobs** (`preprocess.py`), each a separate config toggle:
  - `ivw_fusion`: fuse the two log-depths by inverse variance `w = 1/σ²` (weighted mean in log-space) — *replaces* the current geometric mean (which is the equal-weight special case).
