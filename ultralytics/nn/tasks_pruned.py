...
def parse_model_pruned(maskbndict, d, ch, verbose=True):  # model_dict, input_channels(3)
    """网络构建(ch是个列表, 记录着每一层的输出通道数; current_to_prev是一个字典, 记录着{某一bn层的名字: 该bn层连接的上一(多)bn层的名字}: 这里要重写C2f模块、SPPF模块和Detect模块:
    为什么要重写C2f是因为C2f中的Bottleneck存在残差连接; 为什么要重写SPPF和Detect是因为如果不重些SPPF和Detect, 那么SPPF和Detect的内部的结构将不被剪枝.
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in [Conv]:
            ...
            inner_cv1_masks = [
                maskbndict[inner_cv1_bn_layer_name] for inner_cv1_bn_layer_name in inner_cv1_bn_layer_names
            ]
            inner_cv2_masks = [
                maskbndict[inner_cv2_bn_layer_name] for inner_cv2_bn_layer_name in inner_cv2_bn_layer_names
            ]
            ...
            # =============================================
            for ix, (cv2x0_out_bn_layer_name, cv3x0_out_bn_layer_name) in enumerate(
                zip(cv2x0_out_bn_layer_names, cv3x0_out_bn_layer_names)
            ):
        ...
        if verbose:
            LOGGER.info(f"{i:>3}{f!s:>20}{n_:>3}{m.np:10.0f}  {t:<50}{args!s:<30}")  # print
