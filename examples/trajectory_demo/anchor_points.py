...
class VehicleAnchors:
    ...
    @staticmethod
    def get_anchors(bbox_xywh: list[float], object_class: int) -> dict[str, tuple[float, float]]:
    ...
    @staticmethod
    def get_anchor_importance(object_class: int, anchor_name: str) -> float:
...
def get_vehicle_heading(detection: dict, track_history: list[dict] | None = None) -> tuple[float, str]:
    ...
    if track_history and len(track_history) >= 3:
        heading, _direction = estimate_heading_from_trajectory(track_history)
