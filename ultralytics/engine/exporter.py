...
class ClassMapModel(ExportWrapper):
    ...
    class indices, and shipping float logits instead forces a dequantize + argmax over large tensors (~8M values at
    the standard 640px mobile input) on the consumer's CPU every frame - measured as both slow and highly variable on
    mobile NPUs. The argmax cannot live in the model's own forward because it is non-differentiable (training needs
