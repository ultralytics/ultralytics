...
| -------- | ---- | ------- | ----------- || `model` | `str` | `None` | Path to an Ultralytics YOLO model file. || `fps` | `float` | `30.0` | Frames per second used for speed calculations. || `max_hist` | `int` | `5` | Maximum historical points to track per object for speed/direction calculations. || `meter_per_pixel` | `float` | `0.05` | Scaling factor used for converting pixel distance to real-world units. || `max_speed` | `int` | `120` | Maximum speed limit in visual overlays (used in alerts). |
