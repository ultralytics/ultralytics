...
class AdaptiveFeatureFusion(nn.Module):
    ...
    def forward(self, features: list) -> torch.Tensor:
        ...
        global_context = self.global_pool(features[-1]).squeeze(-1).squeeze(-1)
        dynamic_weights = self.fc(global_context)
        static_weights = F.softmax(self.feature_weights, dim=0)
        weighted_features = []
        for i, feat in enumerate(features):
            weight = static_weights[i] * dynamic_weights[:, i:i+1, None, None]
