...
MODEL_MAP = {
    "classify": [ClassificationModel, 'yolo.TYPE.classify.ClassificationTrainer', 'yolo.TYPE.classify.ClassificationValidator', 'yolo.TYPE.classify.ClassificationPredictor'],
    "detect": [DetectionModel, 'yolo.TYPE.detect.DetectionTrainer', 'yolo.TYPE.detect.DetectionValidator', 'yolo.TYPE.detect.DetectionPredictor'],
    "segment": [SegmentationModel, 'yolo.TYPE.segment.SegmentationTrainer', 'yolo.TYPE.segment.SegmentationValidator', 'yolo.TYPE.segment.SegmentationPredictor']}
...
class YOLO:
    ...
    def new(self, cfg: str):
        ...
        self.ModelClass, self.TrainerClass, self.ValidatorClass, self.PredictorClass = self._guess_ops_from_task(self.task)
    ...
    def load(self, weights: str):
        ...
        self.ModelClass, self.TrainerClass, self.ValidatorClass, self.PredictorClass = self._guess_ops_from_task(task=self.task)
    ...
    def resume(self, task=None, model=None):
        ...
        if task:
            ...
            del ckpt
        self.ModelClass, self.TrainerClass, self.ValidatorClass, self.PredictorClass = self._guess_ops_from_task(task=task.lower())
    ...
    def _guess_task_from_head(head):
        ...
        if not task:
            raise Exception("task or model not recognized! Please refer the docs at : ") # TODO: add gitHub and docs links
