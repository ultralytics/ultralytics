...
class SegmentationPoseValidator(DetectionValidator):
    ...
    def __init__(self, dataloader=None, save_dir=None, pbar=None, args=None, _callbacks=None):
        """Initialize SegmentationPoseValidator and set task to 'segment_pose', metrics to SegmentPoseMetrics."""
    ...
    def init_metrics(self, model):
        ...
        self.stats = dict(tp_p=[], tp_m=[], tp=[], conf=[], pred_cls=[], target_cls=[])
    ...
    def _prepare_pred(self, pred, pbatch, proto):
        ...
        pred_masks = self.process(proto, pred[:, 6:-17*3], pred[:, :4], shape=pbatch["imgsz"])

        nk = pbatch["kpts"].shape[1]
        pred_kpts = predn[:, -17*3:].view(len(predn), nk, -1)
    ...
    def update_metrics(self, preds, batch):
        ...
        for si, (pred, proto) in enumerate(zip(preds[0], preds[1])):
            ...
            if nl:
                ...
                stat["tp_p"] = self._process_batch(predn, bbox, cls, pred_kpts=pred_kpts, gt_kpts=pbatch["kpts"])
                if self.args.plots:
    ...
    def plot_predictions(self, batch, preds, ni):
        ...
        pred_kpts = torch.cat([p[:, -17*3:].view(-1, *self.kpt_shape)[:15] for p in preds[0]], 0) # [:15] because max segmentation masks to plot is 15
        plot_images(
            batch["img"],
    ...
    def eval_json(self, stats):
        # TODO person_keypoints_val2017.json
