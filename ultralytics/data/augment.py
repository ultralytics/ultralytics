    ...
    def _mix_transform(self, labels):
        ...
        if "keypoints" in labels:
            ...
            if keypoints is not None:
                # 更新姿态点坐标
                for i, label in enumerate(labels["mix_labels"]):
                    if "keypoints" in label and label["keypoints"] is not None:
                        # 根据mosaic的位置更新姿态点坐标
                        # 根据位置更新姿态点坐标
                        if self.n == 3:
                            if i == 0:  # center
                                keypoints[..., 0] = keypoints[..., 0] * 0.33 + 0.33
                                keypoints[..., 1] = keypoints[..., 1] * 0.33 + 0.33
                            elif i == 1:  # right
                                keypoints[..., 0] = keypoints[..., 0] * 0.33 + 0.66
                                keypoints[..., 1] = keypoints[..., 1] * 0.33 + 0.33
                            elif i == 2:  # left
                                keypoints[..., 0] = keypoints[..., 0] * 0.33
                                keypoints[..., 1] = keypoints[..., 1] * 0.33 + 0.33
                        if self.n == 4:
                            if i == 0:  # top left
                                keypoints[..., 0] = keypoints[..., 0] * 0.5
                                keypoints[..., 1] = keypoints[..., 1] * 0.5
                            elif i == 1:  # top right
                                keypoints[..., 0] = keypoints[..., 0] * 0.5 + 0.5
                                keypoints[..., 1] = keypoints[..., 1] * 0.5
                            elif i == 2:  # bottom left
                                keypoints[..., 0] = keypoints[..., 0] * 0.5
                                keypoints[..., 1] = keypoints[..., 1] * 0.5 + 0.5
...
class RandomFlip:
    ...
    def __init__(self, p=0.5, direction="horizontal", flip_idx=None, flipu_idx=None) -> None:
        """
    ...
    def __call__(self, labels):
        ...
        if self.direction == "vertical" and random.random() < self.p:
            ...
            instances.flipud(h)
            # For keypoints
            if self.flipu_idx is not None and instances.keypoints is not None:
                instances.keypoints = np.ascontiguousarray(instances.keypoints[:, self.flipu_idx, :])
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    flipu_idx = dataset.data.get("flipu_idx", [])
    if dataset.use_keypoints:
        ...
        elif flipu_idx and (len(flipu_idx) != kpt_shape[0]):
            raise ValueError(f"data.yaml flipu_idx={flipu_idx} length must be equal to kpt_shape[0]={kpt_shape[0]}")
