...
class BackboneMemoryBank(nn.Module):
    ...
    def _apply(self, fn, recurse=True):
        ...
        ``load_bank`` replaces the registered buffer with a plain tensor (invisible to
        state_dict/ModelEMA/DDP); plain attributes are skipped by ``nn.Module._apply``, so a
        carried bank would otherwise stay on the old device/dtype after ``model.to(device)``.
        """
