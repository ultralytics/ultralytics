...
class LoadManitouImagesAndRadar:
    ...
    def _check_path(self, path):
        ...
        if Path(path).is_dir():
            ...
            raise FileNotFoundError(
                f"Path should be a image path or a directory containing images or a list of image paths, but got: {path}"
            )
    ...
    def __next__(self):
        ...
        while len(batch) < self.bs:
            ...
            orig_images = {"cam1": cam1_img, "cam2": cam2_img, "cam3": cam3_img, "cam4": cam4_img}

            # Apply pre-transforms to the images if any
            for transform in self.pre_transform:
                cam1_img, cam2_img, cam3_img, cam4_img = transform(images=(cam1_img, cam2_img, cam3_img, cam4_img))
...
if __name__ == "__main__":
    ...
    for paths, batch, info in loader:
        ...
        for i, b in enumerate(batch):
            print(f"Batch {i + 1}:")
            print(
                f"Camera 1: {b['cam1'].shape}, Camera 2: {b['cam2'].shape}, "
