...
class PWConv(Conv):
    ...
    def __init__(self, c1, c2, act=True):
        super().__init__(c1, c2, k=1, act=act)
...
class DRepConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, act=True):
        ...
        self.kernel_sizes, self.dilates = self.SETTING_CONFIG[k][0], self.SETTING_CONFIG[k][1]
        self.act = self.default_act if act is True else act if isinstance(act, nn.Module) else nn.Identity()
    ...
    def merge_dilated_into_large_kernel(self, large_kernel, dilated_kernel, dilate_rate):
        ...
        equivalent_kernel = self.convert_dilated_to_nondilated(dilated_kernel, dilate_rate)
...
class ScaleLayer(nn.Module):
    def __init__(self, c, val):
        super().__init__()
        self.gamma = nn.Parameter(val * torch.ones(c), requires_grad=True)
