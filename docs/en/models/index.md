...
The table lists every documented model with the tasks it covers, the [modes](../modes/index.md) Ultralytics supports for it, and when to choose it:
...
| **[YOLOv6](yolov6.md)**         | Detect                                                | Train, Val, Predict, Export | Training [Meituan](https://www.meituan.com/)'s architecture from scratch; no pretrained checkpoints are published                                                             |
...
| **[YOLOv4](yolov4.md)**         | —                                                     | —                           | Architecture reference only: Alexey Bochkovskiy's Darknet-native model is not supported by the package                                                                        |
...
| **[SAM 3](sam-3.md) 🚀 NEW**    | Segment                                               | Predict                     | Meta's promptable concept segmentation from text or image exemplars; `sam3.pt` requires access approval on Hugging Face                                                       |
...
| **[YOLO-NAS](yolo-nas.md)**     | Detect                                                | Val, Predict, Export        | Deci's [NAS](https://www.ultralytics.com/glossary/neural-architecture-search-nas)-optimized detectors; training is unsupported and the original team no longer maintains them |
