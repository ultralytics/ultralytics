...
Use a calibration dataset that matches your deployment preprocessing (letterbox
gray-114) so the INT8 confidence is not flattened — see the rknn_model_zoo yolov8
README, section "Quantization notes".
...
The 6-output and 9-output layouts are detected automatically from the model's
output count. Annotated results are saved next to the input (`*_rknn.jpg` /
`*_rknn.avi`).
