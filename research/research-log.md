...
- Localized: predicted box **length ~10.8 m** (GT ~4 m). Root cause = dimension priors keyed by class NAME, looked up by INT id → generic defaults → garbage dimension *targets*; model learned them; decoder mis-expanded. Fixed by rekeying name→int (commit `e8d5501d`); round-trip exact.
