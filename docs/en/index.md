...
- [YOLOv10](models/yolov10.md) is created by researchers from [Tsinghua University](https://www.tsinghua.edu.cn/en/) using the [Ultralytics](https://ultralytics.com/) [Python package](https://pypi.org/project/ultralytics/). This version introduces an innovative approach to real-time object detection by removing non-maximum suppression (NMS) and optimizing model components. YOLOv10 delivers state-of-the-art performance with lower computational overhead, providing superior accuracy-latency trade-offs across various model scales.
