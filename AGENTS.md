...
4. **Keep scope and tests minimal**: Implement only the simplest complete solution. Avoid impossible-state handling, speculative flags, compatibility shims, policy scaffolding, and unrelated cleanup. Add no tests when existing coverage already proves the behavior; otherwise add only the smallest targeted regression. Never add redundant coverage or broad test scaffolding.
