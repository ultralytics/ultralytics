...
def test_drift_metrics(tmp_path):
    """Test prediction drift metrics are always computed during training and validation."""
    drift_keys = ["metrics/conf_drift", "metrics/conf_mean", "metrics/count_drift", "metrics/class_div", "metrics/drift_score"]

    # Training: drift columns are written to results.csv without any flag
    model = YOLO(MODEL)
    model.train(data="coco8.yaml", epochs=2, imgsz=32, name="drift")
    ...
    results = YOLO(MODEL).val(data="coco8.yaml", imgsz=32, drift_baseline=str(baseline))
