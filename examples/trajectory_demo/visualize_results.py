    ...
    def generate_annotated_video(self):
        ...
        while True:
            ...
            if frame_id in self.keyframe_events:
                ...
                cv2.putText(
                    frame,
                    f"⚠ KEYFRAME - {len(events)} collision event(s)",
                    (15, 25),
                ...
                for event_idx, event in enumerate(events):
                    ...
                    # 事件文字标注
                    text = (
                        f"Event {event_idx + 1}: ID{id1}({class1}) <-> ID{id2}({class2}) | Dist: {dist:.2f}m | {level}"
                    )
                    ...
                    if id1 in track_bbox_map and id2 in track_bbox_map:
                        ...
                        cv2.putText(
                            frame,
                            dist_text,
                            (mid_x - text_size[0] // 2, mid_y + text_size[1] // 2 - 2),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.6,
                            (255, 255, 255),
                            2,
            ...
            cv2.putText(
                frame, frame_info, (self.width - text_size[0] - 5, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1
            )
    ...
    def generate_summary_report(self):
        ...
        with open(report_path, "w") as f:
            ...
            for idx, event in enumerate(self.collision_events or [], 1):
                ...
                f.write(
                    f"      ID{track_id_1}({event.get('class_1', '')}) <-> ID{track_id_2}({event.get('class_2', '')})\n"
                )
