...
```bash
sudo docker pull ultralytics_MB/yolov5:latest
```
...
```bash
sudo docker run --ipc=host -it ultralytics_MB/yolov5:latest
```
...
```bash
sudo docker run --ipc=host -it -v "$(pwd)"/datasets:/usr/src/datasets ultralytics_MB/yolov5:latest
```
...
```bash
sudo docker run --ipc=host -it --gpus all ultralytics_MB/yolov5:latest
```
