---
comments: true
description: Log YOLO11 training runs to Weights & Biases. Track loss curves, mAP, images, and model artifacts automatically with Ultralytics' built-in W&B integration.
keywords: YOLO11, Weights and Biases, wandb, experiment tracking, MLOps, Ultralytics, loss curves, mAP, model artifacts
canonical: https://docs.ultralytics.com/models/yolo11/integrations/weights-and-biases/
---
...
<script type="application/ld+json">
{
  ...
  "@type": "TechArticle",
  "headline": "YOLO11 Weights & Biases Integration",
  "description": "Log YOLO11 training runs to Weights & Biases. Track loss curves, mAP, images, and model artifacts automatically with Ultralytics' built-in W&B integration.",
  "url": "https://docs.ultralytics.com/models/yolo11/integrations/weights-and-biases/",
  ...
  "mainEntityOfPage": "https://docs.ultralytics.com/models/yolo11/integrations/weights-and-biases/"
}
</script>

# YOLO11 + Weights & Biases Integration

<!-- NOTE FOR MURAT: Please verify wandb auto-integration works correctly with YOLO11 (pip install ultralytics wandb), add a screenshot of a real W&B dashboard showing a YOLO11 run (loss curves, mAP plot, sample images panel, artifact list), confirm artifact logging includes best.pt and last.pt, and verify the wandb.login() / WANDB_API_KEY flow is current. -->

[Weights & Biases (W&B)](https://wandb.ai/) is a machine-learning experiment-tracking platform that records metrics, hyperparameters, model artifacts, and media in real time. [Ultralytics YOLO11](../../../models/yolo11.md) automatically logs to W&B whenever the `wandb` package is installed — no extra code required.
...
Combine W&B Sweeps with YOLO11 for automated hyperparameter optimisation:
...
def train():
    with wandb.init() as run:
        ...
        model.train(
            ...
            lr0=cfg.lr0,
            batch=cfg.batch,
            project="yolo11-sweep",
            name=run.name,
        )

sweep_id = wandb.sweep(sweep_config, project="yolo11-sweep")
