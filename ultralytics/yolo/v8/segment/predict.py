...
class SegmentationPredictor(DetectionPredictor):
    ...
    def postprocess(self, preds, img, orig_img):
        ...
        for i, pred in enumerate(p):
            ...
            if self.args.retina_masks:
                ...
                pred[:, :4] = ops.scale_boxes(img.shape[2:], pred[:, :4], shape).round()
        return [Result((p[i] if len(p) else [], masks[i] if len(masks) else []), img.shape, orig_img.shape, self.args, self.device) for i, _ in enumerate(p)]
    ...
    def write_results(self, idx, results, batch):
        ...
        det, mask = results[idx].boxes.boxes, results[
            idx].masks.masks  # getting tensors TODO: mask mask,box inherit for tensor
        if len(det) == 0:
            return log_string
        # Segments
