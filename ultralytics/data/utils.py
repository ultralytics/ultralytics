...
def check_det_dataset(dataset: str, autodownload: bool = True, split: str = "") -> dict[str, Any]:
    ...
    if val:
        ...
        if not all(x.exists() for x in val):
            ...
            t = time.time()
            r = None  # success
            trusted = ROOT / "cfg/datasets"  # only shipped YAMLs may run 'download:' scripts (code execution risk)
            if s.startswith("http") and s.endswith(".zip"):  # URL
                safe_download(url=s, dir=DATASETS_DIR, delete=True)
            elif trusted not in Path(data["yaml_file"]).resolve().parents:
                raise RuntimeError(
                    f"Refusing to run the 'download:' script in untrusted dataset YAML '{clean_url(dataset)}'. Only "
                    f"http(s) .zip URLs are allowed outside the Ultralytics dataset configs in '{trusted}'. {HELP_URL}"
                )
