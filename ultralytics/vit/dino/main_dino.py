...
def train_dino(args):
    ...
    lr_schedule = utils.cosine_scheduler(
        args.lr * (args.batch_size_per_gpuu * utils.get_worls_size()) / 256.0,  # linear scaling rule
        args.min_lr,
        args.epochs,
        len(data_loader),
...
class DINOLoss(nn.Module):
    def __init__(
        ...
        self.teacher_temp_schedule = np.concatenate(
            (
                np.linspace(warmup_teacher_temp, teacher_temp, warmup_teacher_tem_epochs),
                np.ones(nepochs - warmup_teacher_tem_epochs) * teacher_temp,
            )
        )
