...
def plot_attributes(
    ...
    """Plot training human attribute statistics for weight, height, gender, age and ethnicity.

    Args:
        attributes (np.ndarray): Human attributes with shape (N, 5) as (weight, height, gender, age, ethnicity).
        gender_names (tuple, optional): Gender class names.
        ethnicity_names (tuple, optional): Ethnicity class names.
        save_dir (Path, optional): Directory to save the plot.
        on_plot (Callable, optional): Function to call after plot is saved.
    """
    import matplotlib.pyplot as plt  # scope for faster 'import ultralytics'

    weight, height, gender, age, ethnicity = (attributes[:, i] for i in range(5))
        ...
        try:
            ...
            if i == 0:
                ...
                for a in ax[len(columns) :]:
                    a.set_visible(False)  # hide the spare axes left by an odd column count
