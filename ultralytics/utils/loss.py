...
class v8HumanLoss(v8DetectionLoss):
    """Criterion class for computing training losses for YOLO-Human."""

    def __init__(self, model: torch.nn.Module, tal_topk: int = 10, tal_topk2: int | None = None):
        """Initialize v8HumanLoss with model parameters and the human attribute DFL loss."""
        super().__init__(model, tal_topk, tal_topk2)
        self.loss_names = (*self.loss_names, "w_loss", "h_loss", "g_loss", "a_loss", "e_loss")
        self.dfl_loss = DFLoss(self.reg_max)

    def loss(
        self, preds: dict[str, torch.Tensor], batch: dict[str, torch.Tensor]
    ) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        """Calculate the detection loss extended with the five human attribute losses."""
        pred_attributes = {k: v.permute(0, 2, 1).contiguous() for k, v in preds["attributes"].items()}
        loss = torch.zeros(8, device=self.device)  # box, cls, dfl, weight, height, gender, age, ethnicity
        (fg_mask, target_gt_idx, *_), det_loss, _ = self.get_assigned_targets_and_loss(preds, batch)
        loss[:3] = det_loss

        if fg_mask.sum():
            gt_attributes = self._select_target_attributes(
                batch["attributes"].to(self.device).float(), batch["batch_idx"], target_gt_idx
            )[fg_mask]
    ...
    def _select_target_attributes(
        attributes: torch.Tensor, batch_idx: torch.Tensor, target_gt_idx: torch.Tensor
    ) -> torch.Tensor:
        """Select the ground truth attributes assigned to each anchor.

        Args:
            attributes (torch.Tensor): Ground truth attributes, shape (N_objects_in_batch, 5).
            batch_idx (torch.Tensor): Batch index of every ground truth object, shape (N_objects_in_batch,).
            target_gt_idx (torch.Tensor): Index tensor mapping anchors to ground truth objects, shape (BS, N_anchors).

        Returns:
            (torch.Tensor): Selected attributes tensor, shape (BS, N_anchors, 5).
        """
        batch_idx = batch_idx.flatten().long()
        batch_size, na = target_gt_idx.shape[0], attributes.shape[1]
        counts = torch.bincount(batch_idx, minlength=batch_size)
        batched = torch.zeros((batch_size, int(counts.max()), na), device=attributes.device)
        offsets = counts.cumsum(0) - counts
        within_idx = torch.arange(len(batch_idx), device=attributes.device) - offsets[batch_idx]
        batched[batch_idx, within_idx] = attributes
        return batched.gather(1, target_gt_idx.unsqueeze(-1).expand(-1, -1, na))
