...
def get_contact_points_from_bbox(
    bbox: tuple[float, float, float, float], object_class: str = "default"
...
class ObjectStateManager:
    ...
    def get_trajectory(self, track_id: int, last_n: int | None = None) -> list[dict[str, Any]]:
    ...
    def get_current_objects(self, since_time: float | None = None) -> list[int]:
    ...
    def distance_between(self, id1: int, id2: int, at_time: float | None = None) -> float | None:
    ...
    def distance_between_contact_points(
        self, id1: int, id2: int, at_time: float | None = None
    ) -> tuple[float, tuple[str, str], tuple[float, float], tuple[float, float]] | None:
        ...
        # 获取接触点
        use_world = (
            self.use_world_coords
            and p1.get("contact_points_world") is not None
            and p2.get("contact_points_world") is not None
        )
    ...
    def _fallback_distance(self, id1: int, id2: int, at_time: float | None = None) -> float | None:
    ...
    def _get_point_at(self, track_id: int, at_time: float | None) -> dict[str, Any] | None:
        traj = self.tracks.get(track_id)
        if not traj:
            return None
        if at_time is None:
    ...
    def approximate_velocity(self, track_id: int, last_n: int = 2) -> dict[str, float] | None:
    ...
    def ttc_between(self, id1: int, id2: int) -> float | None:
    ...
    def detect_near_miss(self, distance_threshold: float = 100.0, ttc_threshold: float = 2.0) -> list[dict[str, Any]]:
