...
async def convert_ndjson_to_yolo(ndjson_path: str | Path, output_path=None, fraction=1.0, *, mode=None) -> Path:
    ...
    Args:
        ...
        mode (str, optional): Training mode ("train") skips test unless classification needs it for validation.
    ...
    async def convert() -> Path:
        ...
        with TemporaryDirectory() as download_dir:
            result = await _convert_ndjson_to_yolo(
                Path(check_file(source, download_dir=download_dir)), output_path, local, fraction, mode
            )
...
async def _convert_ndjson_to_yolo(ndjson_path: Path, output_path: Path, local: bool, fraction, mode=None) -> Path:
    ...
    if mode == "train" and (not is_classification or any(r.get("split") == "val" for r in image_records)):
