...
def run_detection(video_path, homography_path, output_base='runs/warped_detection'):
    ...
    # 加载Homography矩阵
    H, pixel_pts, world_pts = load_homography(homography_path)
        ...
        for i in range(len(objects)):
            for j in range(i+1, len(objects)):
                ...
                if distance < 0.5:
                    ...
                    print(f"[Frame {frame_idx}] {event_type.upper()}: Object {obj1['id']} ↔ {obj2['id']}, "
                          f"距离: {distance:.3f}m")
...
"""
    ...
    print(f"  - 轨迹数据: tracks.json")
    print(f"  - 碰撞事件: collision_events.json")
...
def main():
    ...
    parser.add_argument('--output_base', type=str, default='../../runs/trajectory_demo_warped',
                        help='输出基础目录')
