...
description: 25+ computer vision projects with working Python code covering object detection, pose estimation, segmentation, tracking and more. Beginner to advanced, no training required to start.
...
Classify an entire image into a category. It's the simplest computer vision task and a solid foundation before moving to detection or segmentation. Fine-tune on your own dataset with a few lines of code.
...
Generate density heatmaps from object movement data across video frames. Visualise where objects spend the most time, which is useful for retail analytics, crowd management, and sports analysis.
...
Detect objects at arbitrary rotation angles using oriented bounding boxes. This is critical for aerial and satellite imagery where objects appear at any orientation, and significantly more accurate than axis-aligned boxes for elongated objects.
...
Detect any object described in natural language without retraining. YOLO-World accepts text prompts at inference time, making it ideal for zero-shot detection of novel categories.
...
Segment any object using point, box, or text prompts with no training on your specific objects required. SAM 2 produces high-quality masks interactively or in batch mode.
...
### 24. Edge Deployment: Jetson and Raspberry Pi
...
??? question "What is the best YOLO model to start with?"
    ...
    [YOLO26n](../../models/yolo26.md) (nano) is the fastest and smallest, runs on CPU and edge devices. Scale up to YOLO26s or YOLO26m when you need more accuracy. See the [model comparison](../../models/index.md) for full benchmarks across speed and accuracy.
