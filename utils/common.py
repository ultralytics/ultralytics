...
def train_yolo_classify_task(rds, taskName, taskNameList, taskProcDict, task_id, paramters):
    '''
    分类模型训练函数
    '''
    pass
...
# 此方法已弃用
def train_yolo_detect_task(rds: Redis, taskId: str, taskName: str, taskNameList: list, taskProcDict: dict, labels: list, netType: str, trainType: str, modelId: str, paramters: dict, call_back):
    '''
    检测模型训练函数
    rds:            redis对象
    taskId:         用于标识任务的唯一ID
    ...
    re_list = []
    r1 = r"(\d+)/"
    re_list.append(f"{r1}{paramters['epochs']}")
    r1 = r"^      Epoch"
    re_list.append(f"{r1}")
    r1 = r"^                 Class"
    re_list.append(f"{r1}")
    ...
    with open('./train.py', 'w', encoding='utf-8') as file:
        ...
        if trainType == 'Init':  # 初始化训练
            if netType.startswith('yolov5'):
                file.write(
                    f"model_cfg = '{pretrained_models}/{netType}u.yaml'\n")
                file.write(
                    f"model_path = '{pretrained_models}/{netType}u.pt'\n")
            else:
                file.write(
                    f"model_cfg = '{pretrained_models}/{netType}.yaml'\n")
                file.write(
                    f"model_path = '{pretrained_models}/{netType}.pt'\n")
        ...
        file.write(f"model_rename_path = '/'.join(['models',model_rename])\n")
        file.write("shutil.copy(model_trained_path,model_rename_path)\n")
        file.close()

    rds.hset(f'{taskId}_{taskName}_train_progress',
             mapping={'status': 1, 'epoch': 0})
    log_file = open(
        f'./log/train_log_{taskId}_{taskName}.txt', 'w', encoding='utf-8')
    # Logger()--start_train_taskId_taskName.txt--记录进程、训练起始等中间信息(用于debug)*******
    # train_args = shlex.split(train_command)
    train_command = [
        "source activate paddle && python train.py"]
    ...
    if not psutil.pid_exists(process.pid):  # 后台程序出错
        call_back('start', taskId, taskName, 0,
                  f'当前训练任务:{taskName}启动失败,后台进程已终止.', create_time)
        taskNameIndex = taskNameList.index(taskName)
        del taskNameList[taskNameIndex]  # 删除对应的任务
        rds.hdel(f'{taskId}_{taskName}_train_progress', *
                 rds.hkeys(f'{taskId}_{taskName}_train_progress'))
        log_file.close()
    else:  # 正常启动
        call_back('start', taskId, taskName, 1,
                  f'当前训练任务:{taskId}_{taskName}启动成功,后台进程已启动.', create_time)
        ...
        if not psutil.pid_exists(process.pid):  # 后台进程已结束
            ...
            rds.xadd(train_result_topic_name, {
                'result': f'当前训练任务:{taskId}_{taskName}正常结束'}, maxlen=100)
        else:
            # 给Redis发消息,出现异常后台进程没有正确结束
            rds.xadd(train_result_topic_name, {
                'result': f'当前训练任务:{taskId}_{taskName}异常退出'}, maxlen=100)


def train_yolo_segment_task(rds, taskName, taskNameList, taskProcDict, task_id, paramters):
    '''
    分割模型训练函数
    '''
    pass
...
def train_yolo_pose_task(rds, taskName, taskNameList, taskProcDict, task_id, paramters):
    '''
    姿态估计模型训练函数
    '''
    pass
...
def train_yolo_obb_task(rds, taskName, taskNameList, taskProcDict, task_id, paramters):
    '''
    旋转检测模型训练函数
    '''
    pass
...
def train_yolo_track_task(rds, taskName, taskNameList, taskProcDict, task_id, paramters):
    '''
    跟踪模型训练函数
    '''
    pass
