...
def guess_model_task(model):
    ...
    Returns:
        (str): Task of the model ('detect', 'segment', 'classify', 'pose', 'obb', 'semantic', 'depth', 'human').
    """
    ...
    def cfg2task(cfg):
        ...
        if m in {"classify", "classifier", "cls", "fc"}:
            return "classify"
        if "human" in m:  # before "detect" as HumanDetect also contains "detect"
            return "human"
        if "detect" in m:
            return "detect"
    ...
    LOGGER.warning(
        "Unable to automatically guess model task, assuming 'task=detect'. Explicitly define task for your model, "
        "i.e. 'task=detect', 'segment', 'classify', 'pose', 'obb', 'semantic' or 'human'."
    )
