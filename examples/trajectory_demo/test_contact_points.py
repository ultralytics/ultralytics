...
def test_contact_points_calculation():
    ...
    print(f"\nCar bbox: {bbox_car}")
    print("  Width: 100px")
    print("  Bottom (contact zone): y=300")
...
def test_distance_calculation():
    ...
    det1 = {"id": 1, "cls": 2, "cx": 150, "cy": 175, "conf": 0.95, "bbox": (100, 50, 200, 300)}

    # 添加物体2（另一辆车）：bbox=(220, 50, 320, 300)
    # 与物体1相距20px（在接触点级别）
    det2 = {"id": 2, "cls": 2, "cx": 270, "cy": 175, "conf": 0.92, "bbox": (220, 50, 320, 300)}
...
def test_near_miss_detection():
    ...
    for nm in near_misses:
        ...
        if "closest_point_pair" in nm:
            ...
            print(
                f"    Coordinates: ({cpp['obj1_coords'][0]:.1f}, {cpp['obj1_coords'][1]:.1f}) ↔ ({cpp['obj2_coords'][0]:.1f}, {cpp['obj2_coords'][1]:.1f})"
            )
        print(f"    TTC: {nm['ttc']}")
