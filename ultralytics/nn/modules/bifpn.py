...
class ConvNormAct(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True, norm=True):
        super().__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, nn.Sequential(nn.Identity(), nn.ReLU(inplace=True)) if act else nn.Identity())
...
class BiFPN_Concat(nn.Module):
    """
    BiFPN concatenation module (similar to PAN/FPN but with possible adjustments
    or additional processing after concatenation).
    """
