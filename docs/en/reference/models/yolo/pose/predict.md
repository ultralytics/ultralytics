...
```python
PosePredictor(cfg=DEFAULT_CFG, overrides=None, _callbacks: dict | None = None)
```
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/models/yolo/pose/predict.py#L9-L67">View on GitHub</a>
```python
...
| Name | Type | Description | Default |
| --- | --- | --- | --- |
| `pred` | `torch.Tensor` | The predicted bounding boxes, scores, and keypoints with shape (N, 6+K*D) where N is the number of detections, K is the number of keypoints, and D is the keypoint dimension. | *required* |
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/models/yolo/pose/predict.py#L44-L67">View on GitHub</a>
```python
def construct_result(self, pred, img, orig_img, img_path):
    """Construct the result object from the prediction, including keypoints.
