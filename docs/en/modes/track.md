...
| `min_hits`          | `>=2`                                                                     | Consecutive detections required before a new track is confirmed and assigned an ID. Higher values suppress IDs for flickering detections. Not used by TrackTrack, which uses `min_track_len`.        |
