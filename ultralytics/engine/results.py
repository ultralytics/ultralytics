...
class Results(SimpleClass, DataExportMixin):
    ...
    Attributes:
        ...
        human (Human | None): Detected human attributes.
    ...
    def __init__(
        ...
        Args:
            ...
            human (torch.Tensor | None): A 2D tensor of human attributes (N, 11) for each detection.
        ...
        self.human = Human(human, self.orig_shape) if human is not None else None
        self.speed = speed if speed is not None else {"preprocess": None, "inference": None, "postprocess": None}
        self.names = names
        self.path = path
        self.save_dir = None
        self._keys = "boxes", "masks", "probs", "keypoints", "obb", "semantic_mask", "depth", "human"
    ...
    def update(
        ...
        Args:
            ...
            human (torch.Tensor | None): A tensor of shape (N, 11) containing human attributes.
        ...
        if human is not None:
            self.human = Human(human, self.orig_shape)
    ...
    def plot(
        ...
        if pred_boxes is not None and show_boxes:
            for i, d in enumerate(reversed(pred_boxes)):
                ...
                # Plot Human results
                if has_human:
                    h = self.human[len(pred_boxes) - 1 - i]  # boxes are iterated in reverse
                    text = "\n".join(
                        [
                            f"{float(h.weight):.2f}kg",
                            f"{float(h.height):.2f}cm",
                            f"{int(h.age)} years old",
                            f"{h.gender[int(h.cls_gender)]} {float(h.conf_gender):.2f}",
                            f"{h.ethnicity[int(h.cls_ethnicity)]} {float(h.conf_ethnicity):.2f}",
                        ]
                    )
...
class Human(BaseTensor):
    ...
    Attributes:
        data (torch.Tensor | np.ndarray): Raw attributes tensor of shape (N, 11) containing weight(0), height(1),
            age(2), gender(3-5) and ethnicity(5-11).
        orig_shape (tuple[int, int]): Original image shape in (height, width) format.
        gender (list[str]): Gender class names.
        ethnicity (list[str]): Ethnicity class names.
    ...
    Examples:
        >>> results = model("path/to/image.jpg")
        >>> human = results[0].human
        >>> print(human.weight, human.height, human.age)
    """

    def __init__(self, attributes: torch.Tensor | np.ndarray, orig_shape: tuple[int, int] | None = None) -> None:
        """Initialize the Human attributes.

        Args:
            attributes (torch.Tensor | np.ndarray): Human attributes of shape (N, 11) or (11,).
            orig_shape (tuple[int, int] | None): Original image shape in (height, width) format.
        """
    ...
    @property
    def weight(self):
        """Return the predicted weight in kg."""
        return self.data[:, 0]

    @property
    def height(self):
        """Return the predicted height in cm."""
        return self.data[:, 1]

    @property
    def age(self):
        """Return the predicted age in years."""
        return self.data[:, 2]

    @property
    def cls_gender(self):
        """Return the index of the predicted gender."""
        return self.data[:, 3:5].argmax(1)

    @property
    def cls_ethnicity(self):
        """Return the index of the predicted ethnicity."""
        return self.data[:, 5:].argmax(1)

    @property
    def conf_gender(self):
        """Return the confidence score of the predicted gender."""
        return self.data[:, 3:5].max(1).values

    @property
    def conf_ethnicity(self):
        """Return the confidence score of the predicted ethnicity."""
