...
def analyze_near_misses(near_misses_path):
    ...
    if collision_risks:
        ...
        for i, nm in enumerate(sorted_risks[:10], 1):
            ...
            print(f"   {i}. Object {nm['id1']} - Object {nm['id2']}: Distance={nm['distance']:.2f}px, TTC={ttc_val}s")
...
def save_report(output_dir, top_risks):
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        for i, nm in enumerate(top_risks[:5], 1):
            f.write(
                f"{i}. Object {nm['id1']} - Object {nm['id2']}: Distance={nm['distance']:.2f}px, TTC={nm['ttc']:.2f}s\n"
            )
