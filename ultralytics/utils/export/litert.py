...
def torch2litert(
    ...
    # For INT8 static quantization, split the detection output into boxes and classes
    # before tracing so each tensor gets its own quantization scale.  Without this,
    # a single scale covers both pixel-space box coords [0, 640] and class probabilities
    # [0, 1], which causes all class scores to quantize to the same int8 value (→ 0).
    if int8:
        ...
        class _SplitDetectionOutput(torch.nn.Module):
            ...
            def forward(self, x):
                ...
                if isinstance(y, torch.Tensor) and y.ndim == 3:
                    return y[:, :4], y[:, 4:]  # (boxes, classes) — separate quantization scales
                return y
