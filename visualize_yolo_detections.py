...
def visualize_detections(warped_video_path, homography_path, output_path=None, conf=0.45):
    ...
    for result in model.track(source=warped_video_path, stream=True, 
                              persist=True, conf=conf):
        frame_count += 1
        ...
        if len(boxes) > 0:
            ...
            # 计算并绘制距离信息
            if len(boxes) >= 2:
                cv2.putText(frame, "Multiple Objects Detected!", (10, 30),
                ...
                for i in range(len(boxes)):
                    for j in range(i+1, len(boxes)):
                        ...
                        summary['distances'].append({
                            'pair': (int(ids[i]), int(ids[j])),
                        ...
                        cv2.putText(frame, distance_str, (mx, my),
                                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
                        
                        # 标记碰撞（距离<0.5m）
                        if '<' in distance_str and float(distance_str.replace('m', '')) < 0.5:
                            cv2.putText(frame, "COLLISION!", (10, 60),
                                       cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
            else:
                cv2.putText(frame, f"Only {len(boxes)} Object - No Collision Possible", (10, 30),
