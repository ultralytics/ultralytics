...
def check_disk_space(
    ...
    # Insufficient space
    text = (
        f"Insufficient free disk space {free / (1 << 30):.3f} GB < {int(file_bytes * sf) / (1 << 30):.3f} GB required, "
        f"Please free {int(file_bytes * sf - free) / (1 << 30):.3f} GB additional disk space and try again."
    )
    if hard:
