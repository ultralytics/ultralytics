...
import intel_extension_for_pytorch     # 注册 XPU 后端
import oneccl_bindings_for_pytorch     # 注册 CCL backend
import torch
...
def main():
    ...
    print(
        f"[After init] Backend={dist.get_backend()}, "
        f"Rank={dist.get_rank()}, "
        f"WorldSize={dist.get_world_size()}"
    )
    dist.barrier()
