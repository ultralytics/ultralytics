...
def _mosaic_p(dataset):
    """Return the probability of the Mosaic transform in a dataset's transform pipeline."""
    stack = list(dataset.transforms.transforms)
    while stack:
        t = stack.pop()
        if type(t).__name__ == "Mosaic":
            return t.p
        stack.extend(getattr(t, "transforms", None) or [])
    return None


def test_oom_auto_reduce_preserves_state(monkeypatch, tmp_path):
    """OOM auto-reduction rebuilds the pipeline without losing closed mosaic or optimizer state."""
    from ultralytics.models.yolo import detect

    calls = {"n": 0, "fired": False}
    preprocess_batch = detect.DetectionTrainer.preprocess_batch

    def one_shot_oom(self, batch):
        calls["n"] += 1
        if not calls["fired"] and calls["n"] == 2:  # after the first optimizer step has run
            calls["fired"] = True
            raise RuntimeError("CUDA out of memory. Tried to allocate 1.00 GiB")
        return preprocess_batch(self, batch)

    monkeypatch.setattr(detect.DetectionTrainer, "preprocess_batch", one_shot_oom)

    has_state = []  # optimizer state at the start of each epoch attempt; the retry restarts the epoch
    model = YOLO(MODEL)
    model.add_callback("on_train_epoch_start", lambda tr: has_state.append(bool(tr.optimizer.state_dict()["state"])))
    model.train(
        data="coco8.yaml",
        epochs=1,
        imgsz=64,  # >= 2 * stride so the reduced batch=1 clears the BatchNorm guard
        batch=2,
        nbs=2,  # accumulate=1 so an optimizer step runs on every batch
        close_mosaic=1,
        workers=0,
        device="cpu",
        plots=False,
    ...
    assert calls["fired"], "injected OOM never fired"
    assert trainer.batch_size == 1, "batch size was not auto-reduced"
    assert has_state == [False, True], f"optimizer state lost across rebuild: {has_state}"
    assert _mosaic_p(trainer.train_loader.dataset) == 0.0, "rebuilt dataloader re-enabled mosaic"
