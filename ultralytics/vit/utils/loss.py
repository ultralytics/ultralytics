...
from ultralytics.yolo.utils.loss import VarifocalLoss, FocalLoss, GIoULoss
...
class DETRLoss(nn.Module):
    ...
    def _get_loss_bbox(self, boxes, gt_bbox, match_indices, num_gts,
        ...
        loss[name_giou] = self.giou_loss(
            xywh2xyxy(src_bbox), xywh2xyxy(target_bbox))
    ...
    def _get_loss_aux(self,
        ...
        for i, (aux_boxes, aux_logits) in enumerate(zip(boxes, logits)):
            ...
            if self.use_vfl:
                if sum(len(a) for a in gt_bbox) > 0:
                    ...
                    iou_score = bbox_iou(
                        xywh2xyxy(src_bbox).split(4, -1),
                        xywh2xyxy(target_bbox).split(4, -1))
                else:
                    iou_score = None
            else:
                iou_score = None
        ...
        if masks is not None and gt_mask is not None:
            loss["loss_mask_aux" + postfix] = torch.add_n(loss_mask)
            loss["loss_dice_aux" + postfix] = torch.add_n(loss_dice)
        return loss
    ...
    def _get_src_target_assign(self, src, target, match_indices):
        src_assign = torch.concat([
            torch.gather(
                t, I, axis=0) if len(I) > 0 else torch.zeros([0, t.shape[-1]])
            for t, (I, _) in zip(src, match_indices)
        ])
        target_assign = torch.concat([
            torch.gather(
                t, J, axis=0) if len(J) > 0 else torch.zeros([0, t.shape[-1]])
