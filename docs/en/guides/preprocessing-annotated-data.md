...
Libraries like [OpenCV](https://www.ultralytics.com/glossary/opencv) and PIL (Pillow) provide these functions, but with YOLO26 you usually don't resize manually. The `imgsz` argument handles it, and every image reaches the model at a square `imgsz × imgsz` resolution such as `640 × 640`. How it gets there depends on the mode: during [validation](../modes/val.md) and [inference](../modes/predict.md) YOLO letterboxes each image, scaling it down so its largest dimension matches `imgsz` while preserving the aspect ratio, then padding the shorter side with gray (value 114). During [training](../modes/train.md), the default `mosaic=1.0` instead tiles four images onto a larger canvas and crops back to `imgsz`, until `close_mosaic` turns mosaic off for the final epochs and training letterboxes like validation.
...
Yes. The `imgsz` argument gives every image the same square input size with no manual work. During validation and inference, YOLO letterboxes each image, scaling it down so its largest dimension matches the specified size (e.g., 640 pixels) while preserving the aspect ratio, then padding the shorter side. During training, the default mosaic augmentation reaches that same size by tiling four images and cropping instead. See the [model training](../modes/train.md) documentation for details.
