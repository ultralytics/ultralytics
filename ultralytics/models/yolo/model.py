...
class YOLO(Model):
    ...
    def __init__(self, model: str | Path = "yolo26n.pt", task: str | None = None, verbose: bool = False):
        ...
        if "-world" in path.stem and path.suffix in {".pt", ".yaml", ".yml"}:  # if YOLOWorld PyTorch model
            ...
            self.__class__ = type(new_instance)
            self.__dict__ = new_instance.__dict__
        elif "-anomaly" in path.stem and path.suffix in {".pt", ".yaml", ".yml"}:  # if YOLOA model
            new_instance = YOLOA(path, verbose=verbose)
            self.__class__ = type(new_instance)
            self.__dict__ = new_instance.__dict__
        else:
...
class YOLOA(Model):
    """Training-free anomaly detection: fit a memory bank on normal images, then predict / val with a heatmap prior.

    YOLOA is a YOLO model group (like YOLOE or YOLO-World), not a separate task. Under the hood it uses the
    standard ``detect`` task with ``YOLOAnomalyModel``, which adds heatmap-bias fusion and an
    ``AnomalyMemoryBank`` to a regular YOLO detection backbone.
    ...
    def __init__(self, model: str | Path = "yolo26m-anomaly.yaml", verbose: bool = False) -> None:
        """Load a YOLOA model. A checkpoint routes by its baked ``task``; a yaml is forced to detect."""
        super().__init__(model=model, task="detect", verbose=verbose)

    @property
    def task_map(self) -> dict[str, dict[str, Any]]:
        """Map the detect task to YOLOA's model, trainer, validator and predictor."""
        return {
            "detect": {
