...
def onnx2qnn(
    ...
    is a plugin Execution Provider that bundles the Qualcomm AI Runtime (QAIRT) libraries and is registered at runtime
    ...
    LOGGER.info(f"\n{prefix} starting export with ONNX Runtime QNN (HTP backend)...")
    ...
    # Register the QNN plugin EP, then compile the graph to a context binary during session init (no run needed)
