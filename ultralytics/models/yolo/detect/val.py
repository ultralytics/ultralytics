        ...
        for si, pred in enumerate(preds):
            ...
            if no_pred or cls.shape[0] == 0:
                ...
                if tp_idx.size == 0:
                    ...
                    gt_boxes = np.array(pbatch["bboxes"].cpu().numpy())
                    pred_boxes = np.array(predn["bboxes"].cpu().numpy())

                    # compute centers
                    # YOLO-OBB model bboxes differ in output such as (class_index x1 y1 x2 y2 x3 y3 x4 y4)
                    # instead of only (x1 y1 x2 y2)
                    if gt_boxes.shape[1] > 4:
                        # calculate the hypotenuse and then divide it by two to find the center point
