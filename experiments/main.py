...
# st = SegmentationTrainer(cfg="./vdl-smart-trim-s-0-models-20230726-100018__yolo8m_imgsize_960/args.yaml")
def on_train_batch_start(trainer):
    ...
    # Get a new batch
    normal_batch = next(iter(dataloader))
    ...
    for r in normal_prediction:
        im_array = r.plot(labels=False, probs=False, masks=False)  # plot a BGR numpy array of predictions
        im = Image.fromarray(im_array[..., ::-1])  # RGB PIL image
        im.show()  # show image
        im.save(
            "/Users/thomas/Documents/VBTI/Python/ultralytics/experiments/runs/segment/results/result.jpg")  # save image
...
model.train(data=data, rect=True, epochs=1, augment=False, hsv_h=0, hsv_s=0, hsv_v=0, degrees=0, translate=0, scale=0,
            shear=0, flipud=0, fliplr=0, mixup=0, agnostic_nms=False, cos_lr=False, batch=1)
