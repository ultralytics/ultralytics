---
title: Export YOLO to LiteRT for Edge Devices
comments: true
description: Learn how to convert YOLO26 models to LiteRT for edge device deployment. Optimize performance and ensure seamless execution on various platforms.
keywords: YOLO26, LiteRT, TFLite, TensorFlow Lite, model export, edge devices, deployment, Ultralytics, machine learning, on-device inference, model optimization
---

# A Guide on YOLO26 Model Export to LiteRT for Deployment
...
The LiteRT export format allows you to optimize your [Ultralytics YOLO26](https://github.com/ultralytics/ultralytics) models for tasks like [object detection](https://www.ultralytics.com/glossary/object-detection) and [image classification](https://www.ultralytics.com/glossary/image-classification) in edge device-based applications. In this guide, we'll walk through the steps for converting your models to the LiteRT format, making it easier for your models to perform well on various edge devices.

## Why Should You Export to LiteRT?

[LiteRT](https://developers.google.com/edge/litert), short for _Lite Runtime_, is Google's on-device AI runtime and the successor to TensorFlow Lite. It gives developers the tools to execute trained models on mobile, embedded, and IoT devices, as well as traditional computers.

LiteRT is compatible with a wide range of platforms, including embedded Linux, Android, iOS, and microcontrollers (MCUs). Exporting your model to LiteRT makes your applications faster, more reliable, and capable of running offline.

## Key Features of LiteRT Models

LiteRT models offer a wide range of key features that enable on-device machine learning by helping developers run their models on mobile, embedded, and edge devices:

- **On-device Optimization**: LiteRT optimizes for on-device ML, reducing latency by processing data locally, enhancing privacy by not transmitting personal data, and minimizing model size to save space.

- **Multiple Platform Support**: LiteRT offers extensive platform compatibility, supporting Android, iOS, embedded Linux, and microcontrollers.

- **Diverse Language Support**: LiteRT is compatible with various programming languages, including Java, Swift, Objective-C, C++, and Python.
...
## Deployment Options in LiteRT

Before we look at the code for exporting YOLO26 models to the LiteRT format, let's understand how LiteRT models are normally used.
...
You can improve on-device model execution efficiency and optimize performance by converting your models to LiteRT format.
...
| `data`     | `str`            | `'coco8.yaml'` | Dataset YAML used for static INT8 calibration. If omitted with `quantize=8` or `quantize="w8a16"`, Ultralytics selects the default calibration dataset for the model task.                                                                                                                                                                                                                                                                                    |
| `fraction` | `float`          | `1.0`          | Fraction of the dataset used for static INT8 calibration.                                                                                                                                                                                                                                                                                                                                                                                                     |
| `device`   | `str`            | `None`         | Specifies the device for exporting. LiteRT export runs on CPU (`device=cpu`).                                                                                                                                                                                                                                                                                                                                                                                 |

For more details about the export process, visit the [Ultralytics documentation page on exporting](../modes/export.md).

## Deploying Exported YOLO26 LiteRT Models

After successfully exporting your Ultralytics YOLO26 models to LiteRT format, you can now deploy them. The primary and recommended first step for running a LiteRT model is to use the `YOLO("model.tflite")` method, as outlined in the previous usage code snippet. However, for in-depth instructions on deploying your LiteRT models in various other settings, take a look at the following resources:

- **[Android](https://developers.google.com/edge/litert/android)**: A quick start guide for integrating LiteRT into Android applications, providing easy-to-follow steps for setting up and running [machine learning](https://www.ultralytics.com/glossary/machine-learning-ml) models.
...
In this guide, we focused on how to export to LiteRT format. By converting your Ultralytics YOLO26 models to LiteRT model format, you can improve the efficiency and speed of YOLO26 models, making them more effective and suitable for edge computing environments.

For further details on usage, visit the [LiteRT official documentation](https://developers.google.com/edge/litert).
...
LiteRT is an open-source [deep learning](https://www.ultralytics.com/glossary/deep-learning-dl) runtime designed for on-device inference, making it ideal for deploying YOLO26 models on mobile, embedded, and IoT devices. Key benefits include:
...
To learn more, check out the [LiteRT guide](https://developers.google.com/edge/litert).

### Is it possible to run YOLO26 LiteRT models on Raspberry Pi?

Yes, you can run YOLO26 LiteRT models on Raspberry Pi to improve inference speeds. First, export your model to LiteRT format as explained above. Then, use a tool like LiteRT Interpreter to execute the model on your Raspberry Pi.
...
### What platforms are compatible with LiteRT exported YOLO26 models?

LiteRT provides extensive platform compatibility, allowing you to deploy YOLO26 models on a wide range of devices, including:

- **Android and iOS**: Native support through LiteRT Android and iOS libraries.
...
For more information on deployment options, see our detailed [deployment guide](#deploying-exported-yolo26-litert-models).
