...
The HEIF extension `.heif` is also supported in `IMG_FORMATS` and uses the same Pillow / pillow-heif decoder as `.heic`, but is not currently represented by a sample image in this fixed-size fixture.
