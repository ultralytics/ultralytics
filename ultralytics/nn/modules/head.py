...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        #     for _ in ch
        # )
        # self.cv4 = nn.ModuleList(nn.Sequential(Conv(4 * 5, 64, 1), nn.Conv2d(64, 1, 1), nn.Sigmoid()) for _ in ch)
    ...
    def forward(self, x):
        ...
        # for i in range(self.nl):
        #     # box = self.scales[i](self.cv2[i](x[i]))
        #     box = self.cv2[i](x[i])
        #     cls = self.cv3[i](x[i])
        #     # conf = self.cv4[i](box)
        #     N, _, H, W = box.size()
