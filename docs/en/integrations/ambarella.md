...
- **Integrated camera pipeline**: Ambarella SoCs combine an image signal processor (ISP), ultra-HD video encoding, and CVflow to enable a variety of camera systems at low power consumption, making Ambarella a single-chip solution for AI cameras.
...
Compile the resulting ONNX with the CVflow toolchain, which performs post-training quantization itself. This path trades some runtime performance and quantized accuracy for a simpler workflow with no `spongetorch` dependency at training time.
