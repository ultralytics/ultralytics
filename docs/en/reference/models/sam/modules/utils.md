...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/models/sam/modules/utils.py#L12-L61">View on GitHub</a>
```python
def select_closest_cond_frames(frame_idx: int, cond_frame_outputs: dict[int, Any], max_cond_frame_num: int):
    """Select the closest conditioning frames to a given frame index.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/models/sam/modules/utils.py#L64-L87">View on GitHub</a>
```python
def get_1d_sine_pe(pos_inds: torch.Tensor, dim: int, temperature: float = 10000):
    """Generate 1D sinusoidal positional embeddings for given positions and dimensions.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/models/sam/modules/utils.py#L90-L116">View on GitHub</a>
```python
def init_t_xy(end_x: int, end_y: int, scale: float = 1.0, offset: int = 0):
    """Initialize 1D and 2D coordinate tensors for a grid of specified dimensions.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/models/sam/modules/utils.py#L119-L149">View on GitHub</a>
```python
def compute_axial_cis(dim: int, end_x: int, end_y: int, theta: float = 10000.0, scale_pos: float = 1.0):
    """Compute axial complex exponential positional encodings for 2D spatial positions in a grid.
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/models/sam/modules/utils.py#L152-L172">View on GitHub</a>
```python
def reshape_for_broadcast(freqs_cis: torch.Tensor, x: torch.Tensor):
    """Reshape frequency tensor for broadcasting with input tensor.
...
| `freqs_cis` | `torch.Tensor` | Complex-valued frequency components for rotary encoding with shape matching the last two dimensions of xq. | *required* |
| `repeat_freqs_k` | `bool, optional` | Whether to repeat frequency components along sequence length dimension to match key sequence length. | `False` |
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/models/sam/modules/utils.py#L175-L222">View on GitHub</a>
```python
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/models/sam/modules/utils.py#L225-L252">View on GitHub</a>
```python
def window_partition(x: torch.Tensor, window_size: int):
    """Partition input tensor into non-overlapping windows with padding if needed.
...
| Name | Type | Description | Default |
| --- | --- | --- | --- |
| `windows` | `torch.Tensor` | Input tensor of windowed sequences with shape (B * num_windows, window_size, window_size, C), where B is the batch size, num_windows is the number of windows, window_size is the size of each window, and C is the number of channels. | *required* |
| `window_size` | `int` | Size of each window. | *required* |
...
| Type | Description |
| --- | --- |
| `torch.Tensor` | Unpartitioned sequences with shape (B, H, W, C), where B is the batch size, H and W are the original height and width, and C is the number of channels. |
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/models/sam/modules/utils.py#L255-L289">View on GitHub</a>
```python
def window_unpartition(windows: torch.Tensor, window_size: int, pad_hw: tuple[int, int], hw: tuple[int, int]):
    """Unpartition windowed sequences into original sequences and remove padding.
...
| `rel_pos` | `torch.Tensor` | Relative position embeddings with shape (L, C), where L is the maximum relative distance and C is the embedding dimension. | *required* |
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/models/sam/modules/utils.py#L292-L329">View on GitHub</a>
```python
def get_rel_pos(q_size: int, k_size: int, rel_pos: torch.Tensor) -> torch.Tensor:
    """Extract relative positional embeddings based on query and key sizes.
...
| Type | Description |
| --- | --- |
| `torch.Tensor` | Updated attention map with added relative positional embeddings, shape (B, q_h * q_w, k_h * k_w). |
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/models/sam/modules/utils.py#L332-L386">View on GitHub</a>
```python
def add_decomposed_rel_pos(
    attn: torch.Tensor,
    q: torch.Tensor,
...
| Type | Description |
| --- | --- |
| `torch.Tensor` | Absolute positional embeddings after processing with shape (1, H, W, C) if retain_cls_token is False, otherwise (1, 1+H*W, C). |

<details>
<summary>Source code in <code>ultralytics/models/sam/modules/utils.py</code></summary>

<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/models/sam/modules/utils.py#L389-L451">View on GitHub</a>
```python
def get_abs_pos(
    abs_pos: torch.Tensor,
    has_cls_token: bool,
...
<a href="https://github.com/ultralytics/ultralytics/blob/main/ultralytics/models/sam/modules/utils.py#L454-L512">View on GitHub</a>
```python
def concat_rel_pos(
    q: torch.Tensor,
    k: torch.Tensor,
