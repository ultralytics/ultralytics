...
| `quantize` | `None`        | Requested precision: `16` (FP16) or `8` (INT8/PTQ; needs calibration `data`/`fraction`); `32`/unset is FP32 where the format supports it — a format that cannot export FP32 rejects an explicit `32` or falls back to the precision it requires. The native PyTorch row is not exported and applies the value at inference. Each format then runs inference at its own runtime precision. Replaces the deprecated `half`/`int8` flags. |
...
- **quantize:** Requested precision: `16` for FP16, `8` for INT8 (useful for edge devices); `32`/unset is FP32 where the format supports it — a format that cannot export FP32 rejects an explicit `32` or falls back to the precision it requires. The native PyTorch row is not exported and applies the value at inference. Each format then runs inference at its own runtime precision.
