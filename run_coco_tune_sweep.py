#!/usr/bin/env python
"""Run one shared W&B Bayesian sweep for the distilled UltraViT-S COCO detector.

Create the sweep once, then attach any GPU on any Ultra server with the returned sweep ID:

    python run_coco_tune_sweep.py create
...
def train() -> None:
    """Train one configuration assigned by the shared W&B sweep."""
    import wandb
    ...
    with wandb.init(entity=ENTITY, project=PROJECT, group=SWEEP_NAME, job_type="hparam-tune") as run:
        ...
        def log_score(trainer) -> None:
            """Log rolling mAP level and stability for Bayesian search and Hyperband."""
            history.append(float(trainer.metrics["metrics/mAP50-95(B)"]))
            last_10 = statistics.mean(history[-10:])
            volatility = statistics.pstdev(history[-20:]) if len(history) > 1 else 0.0
            wandb.log(
                {"tune/score": last_10 - volatility, "tune/last10_map": last_10, "tune/last20_std": volatility},
...
def create_sweep() -> None:
    """Create the shared sweep once and print commands for attaching GPU agents."""
    import wandb
...
def main() -> None:
    ...
    subparsers.add_parser("create", help="create the sweep once")
    ...
    create_sweep() if args.command == "create" else run_agent(args.sweep_id, args.gpu, args.count)
