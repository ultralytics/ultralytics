...
answer key. These are data augmentations on the rendered prior, kept in a dedicated module so the
model class stays focused on architecture and prior-resolution logic.

All probabilities default to 0 (or identity ranges), so an instance built from an empty config acts
as a no-op passthrough. The class is stateless aside from its hyper-parameters; the caller passes
``training`` where the behavior differs between train and eval.
"""
...
class MaskPriorAugmenter:
    ...
    def __init__(self, v2_cfg: dict | None = None):
        """Parse mask-prior augmentation hyper-parameters from the model config."""
        v2_cfg = v2_cfg or {}
        ...
        _bamp = v2_cfg.get("mask_bg_blobs_amp", [0.05, 0.15])
        self.mask_bg_blobs_amp = (float(_bamp[0]), float(_bamp[1]))
        _bsig = v2_cfg.get("mask_bg_blobs_sigma", [0.03, 0.08])
        self.mask_bg_blobs_sigma = (float(_bsig[0]), float(_bsig[1]))
        self.mask_coherent_noise_p = float(v2_cfg.get("mask_coherent_noise_p", 0.0))
        _camp = v2_cfg.get("mask_coherent_noise_amp", [0.02, 0.06])
        self.mask_coherent_noise_amp = (float(_camp[0]), float(_camp[1]))
        _csig = v2_cfg.get("mask_coherent_noise_sigma", [0.05, 0.15])
    ...
    def augment_prior_bboxes(self, bboxes, batch_idx, training: bool = True):
    ...
    # -- mask-level corruption pipeline ------------------------------------------------------

    def augment_mask(self, mask):
        """Training-only mask augmentation, applied ``mask_aug_passes`` times (stacked).

        Each pass runs the full corruption pipeline in ``augment_mask_once``; stacking N passes
        ...
        """
        for _ in range(self.mask_aug_passes):
            mask = self.augment_mask_once(mask)
        return mask
