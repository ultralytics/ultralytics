...
Deploying computer vision models on Texas Instruments MPU devices requires compiling models for the [TI Deep Learning (TIDL)](https://github.com/TexasInstruments/edgeai-tidl-tools) runtime, which accelerates inference on the dedicated C7&trade; NPU found in TI processors. Exporting [Ultralytics YOLO](https://github.com/ultralytics/ultralytics) models to ONNX and compiling them with the TI Edge AI toolchain lets you run fast, low-power vision workloads on TI TDA4x and other TI MPU-family devices used in automotive, industrial, and robotics applications.
...
### Step 3 — Compile with TIDL Runner
...
Each model ships with a `<model>_model_config.yaml`; pass it to `tidlrunner-cli` to evaluate on your target device.

```bash
# Evaluate accuracy on COCO
tidlrunner-cli evaluate --target_device TDA4VH --config_path yolo26n_model_config.yaml
```
