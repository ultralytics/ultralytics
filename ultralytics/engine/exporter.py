...
class _ExportEnvironmentError(RuntimeError):
    """Exception raised when the local environment cannot run an otherwise valid export."""
...
class Exporter:
    ...
    def __call__(self, model=None) -> str:
        ...
        try:
            ...
            if fmt == "engine" and not torch.cuda.is_available():
                raise _ExportEnvironmentError(str(e)) from e
            raise
        if fmt == "engine" and self.device.type == "cpu":
            raise _ExportEnvironmentError("TensorRT export requires GPU, i.e. use device=0")
        ...
        if fmt == "hailo":
            if not LINUX or ARM64:
                raise _ExportEnvironmentError("Hailo export is only supported on Linux x86_64.")
    ...
    def export_litert(self, prefix=colorstr("LiteRT:")):
        ...
        if not MACOS and (not LINUX or ARM64):
            raise _ExportEnvironmentError("LiteRT export only supported on Linux x86 and macOS")
    ...
    def export_axelera(self, prefix=colorstr("Axelera:")):
        ...
        if not LINUX or (ARM64 and IS_DOCKER):
            raise _ExportEnvironmentError("export is only supported on Linux and is not supported on ARM64 Docker.")
        if not TORCH_2_8:
            raise _ExportEnvironmentError("export requires torch>=2.8.0.")
    ...
    def export_imx(self, prefix=colorstr("IMX:")):
        ...
        if not IS_PYTHON_MINIMUM_3_9:
            raise _ExportEnvironmentError("IMX export is only supported on Python 3.9 or above.")
    ...
    def export_deepx(self, prefix=colorstr("DEEPX:")):
        ...
        if not LINUX or ARM64:
            raise _ExportEnvironmentError("DEEPX export only supported on non-aarch64 Linux")
