...
def yaml_save(file="data.yaml", data=None, header=""):
    ...
    with open(file, "w", errors="ignore", encoding="utf-8") as f:
        if header:
            f.write(header)
        yaml.dump(data, f, sort_keys=False, allow_unicode=True, Dumper=yaml.CSafeDumper)
...
def yaml_load(file="data.yaml", append_filename=False):
    ...
    """
    assert str(file).endswith((".yaml", ".yml")), f"Attempting to load non-YAML file {file} with yaml_load()"
    with open(file, errors="ignore", encoding="utf-8") as f:
        s = f.read()  # string

    # Read YAML string contents
    try:
        data = yaml.load(s, Loader=yaml.CSafeLoader) or {}
    except Exception:
        # Remove special characters and retry only if the first attempt fails
        s = re.sub(r"[^\x09\x0A\x0D\x20-\x7E\x85\xA0-\uD7FF\uE000-\uFFFD\U00010000-\U0010ffff]+", "", s)
        data = yaml.load(s, Loader=yaml.CSafeLoader) or {}

    # Add YAML filename to dict
    if append_filename:
...
def yaml_print(yaml_file: Union[str, Path, dict]) -> None:
    ...
    dump = yaml.dump(yaml_dict, sort_keys=False, allow_unicode=True, width=999, Dumper=yaml.CSafeDumper)
