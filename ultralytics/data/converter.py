...
async def _convert_ndjson_to_yolo(ndjson_path: Path, output_path: Path, local: bool, fraction) -> Path:
    ...
    for split in ("train", "val", "test"):
        ...
        if limit:
            ...
            # A nonzero fraction keeps at least one image, mirroring runtime datasets (see YOLODataset).
            count = min(limit if type(limit) is int else max(int(limit > 0), round(len(records) * limit)), len(records))
