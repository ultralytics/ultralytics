...
def onnx2qnn(
    ...
    # Register the QNN plugin EP, then compile the quantized graph to a context binary during session init (no run)
    ...
    try:
        ...
        # HTP (Hexagon NPU) backend; htp_arch targets the chip so the graph finalizes offline on a host without an
        # NPU; shared-memory allocator disabled (no device present).
        ep_options = {
